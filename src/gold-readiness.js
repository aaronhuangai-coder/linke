...
const GOLD_READINESS_ITEMS = [
  ...
  },
  {
    ...
    nextStep: 'V0.80 adds a sanitized supervisor install readiness gate and blocked --fail-on-blocked automation signal but still only reports not_configured supervisor state; add real installer, launchd install/start, watchdog, monitoring, and managed daemon lifecycle only after auth and production boundaries are designed.',
  },
  {
    id: 'security-auth',
    area: 'security',
  ...
  },
  {
    ...
    nextStep: 'Recent releases add read-only Agent/Web visibility, supervisor-status not_configured reporting, supervisor install dry-run planning, supervisor install readiness gating, and blocker gates; still add deployment hardening, audit rotation, tamper-proof audit storage, distributed rate limiting, secret management, monitoring, and recovery supervisor.',
  },
];
