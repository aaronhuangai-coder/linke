...
describe('README — V1.13 Supervisor lifecycle guarded runner execution gate', () => {
  it('title and badge claim V1.13 as current', () => {
    assertReadmeContains(/# Linke V1\.13/, 'README title should mention V1.13');
    assertReadmeContains(/\*\*当前版本：V1\.13\*\*/, 'README badge should mention V1.13');
  ...
  it('version table marks V1.13 current and keeps V1.12/V1.11/V1.10/V1.09 historical', () => {
    assertReadmeContains(/\| V1\.13 \| 当前版本 \|[^|]*buildSupervisorLifecycleGuardedRunnerExecutionGate[^|]*test\/supervisor-lifecycle-guarded-runner-execution-gate\.test\.js[^|]*executionEligible:false[^|]*execute-request-missing[^|]*realRunnerWiringReady:false[^|]*real-guarded-runner-execution-wiring-missing/i, 'V1.13 should be current guarded runner execution gate milestone');
  ...
  it('documents executor manifest readiness pure function and fail-closed boundaries', () => {
    ...
    assertReadmeContains(/executionEligible:false[\s\S]*execute-request-missing[\s\S]*realRunnerWiringReady:false/i, 'README should document V1.13 execution gate blocked flags');
  ...
  it('keeps Gold blocked and rejects lifecycle overclaims for V1.13', () => {
  ...
  it('keeps later Gold readiness sections synced to V1.13 evidence', () => {
    assertReadmeContains(/Agent CLI[\s\S]*?V1\.13[\s\S]*?buildSupervisorLifecycleGuardedRunnerExecutionGate[\s\S]*?supervisor-lifecycle-guarded-runner-execution-gate[\s\S]*?test\/supervisor-lifecycle-guarded-runner-execution-gate\.test\.js[\s\S]*?V1\.12[\s\S]*?supervisor-lifecycle-guarded-runner-execution-preview-button[\s\S]*?buildSupervisorLifecycleGuardedRunnerExecutionPreviewViewModel[\s\S]*?test\/web-console\.test\.js supervisor lifecycle guarded runner execution preview[\s\S]*?V1\.11[\s\S]*?POST \/api\/supervisor-lifecycle-guarded-runner-execution-preview[\s\S]*?test\/supervisor-lifecycle-guarded-runner-execution-preview-api\.test\.js[\s\S]*?V1\.10[\s\S]*?agent\.js supervisor-lifecycle-guarded-runner-execution-preview[\s\S]*?test\/agent-supervisor-lifecycle-guarded-runner-execution-preview\.test\.js[\s\S]*?V1\.09[\s\S]*?buildSupervisorLifecycleGuardedRunnerExecutionPreview[\s\S]*?test\/supervisor-lifecycle-guarded-runner-execution-preview\.test\.js/i, 'README Agent CLI Gold section should mention V1.13 gate evidence, V1.12 Web evidence, V1.11 API evidence, V1.10 guarded runner execution preview CLI evidence and V1.09 pure evidence');
    ...
    assertReadmeContains(/Gold blockers[\s\S]*?V1\.13[\s\S]*?buildSupervisorLifecycleGuardedRunnerExecutionGate[\s\S]*?test\/supervisor-lifecycle-guarded-runner-execution-gate\.test\.js[\s\S]*?executionEligible:false[\s\S]*?execute-request-missing[\s\S]*?realRunnerWiringReady:false[\s\S]*?V1\.12[\s\S]*?supervisor-lifecycle-guarded-runner-execution-preview-button[\s\S]*?buildSupervisorLifecycleGuardedRunnerExecutionPreviewViewModel[\s\S]*?test\/web-console\.test\.js supervisor lifecycle guarded runner execution preview[\s\S]*?V1\.11[\s\S]*?POST \/api\/supervisor-lifecycle-guarded-runner-execution-preview[\s\S]*?test\/supervisor-lifecycle-guarded-runner-execution-preview-api\.test\.js[\s\S]*?V1\.10[\s\S]*?agent\.js supervisor-lifecycle-guarded-runner-execution-preview[\s\S]*?test\/agent-supervisor-lifecycle-guarded-runner-execution-preview\.test\.js[\s\S]*?V1\.09[\s\S]*?buildSupervisorLifecycleGuardedRunnerExecutionPreview[\s\S]*?guarded-runner-execution-preview-only[\s\S]*?real-guarded-runner-execution-wiring-missing[\s\S]*?test\/supervisor-lifecycle-guarded-runner-execution-preview\.test\.js/i, 'README Gold blockers section should mention V1.13 gate evidence, V1.12 Web evidence, V1.11 API evidence, V1.10 CLI evidence and V1.09 pure evidence');
