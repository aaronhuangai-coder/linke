...
describe('README — V1.05 Supervisor lifecycle guarded runner readiness', () => {
  it('title and badge claim V1.05 as current', () => {
    assertReadmeContains(/# Linke V1\.05/, 'README title should mention V1.05');
    assertReadmeContains(/\*\*当前版本：V1\.05\*\*/, 'README badge should mention V1.05');
    assertReadmeDoesNotContain(/^# Linke V1\.04/m, 'README title must not still claim V1.04');
    assertReadmeDoesNotContain(/\*\*当前版本：V1\.04\*\*/, 'README badge must not still claim V1.04');
  });

  it('version table marks V1.04 historical and V1.05 current', () => {
    assertReadmeContains(/\| V1\.05 \| 当前版本 \|[^|]*buildSupervisorLifecycleGuardedRunnerReadiness[^|]*supervisor-lifecycle-guarded-runner-binding[^|]*runnerBindingsReady:true[^|]*executorReady:false[^|]*guarded-runner-execution-disabled[^|]*wouldRun:false[^|]*wouldWrite:false[^|]*test\/supervisor-lifecycle-guarded-runner-readiness\.test\.js/i, 'V1.05 should be current supervisor lifecycle guarded runner readiness milestone');
  ...
  it('keeps Gold blocked and rejects lifecycle overclaims for V1.05', () => {
  ...
  it('keeps later Gold readiness sections synced to V1.05 evidence', () => {
    assertReadmeContains(/Agent CLI[\s\S]*?V1\.05[\s\S]*?buildSupervisorLifecycleGuardedRunnerReadiness[\s\S]*?test\/supervisor-lifecycle-guarded-runner-readiness\.test\.js/i, 'README Agent CLI Gold section should mention V1.05 guarded runner readiness evidence');
    ...
    assertReadmeContains(/NAS、认证和生产硬化仍是 partial[\s\S]*?V1\.05[\s\S]*?buildSupervisorLifecycleGuardedRunnerReadiness[\s\S]*?V1\.04[\s\S]*?supervisor-lifecycle-executor-manifest-readiness-button[\s\S]*?V1\.03[\s\S]*?POST \/api\/supervisor-lifecycle-executor-manifest-readiness/i, 'README partial hardening section should mention V1.05 runner evidence, retain V1.04 manifest Web evidence and V1.03 API evidence');
  });
});
