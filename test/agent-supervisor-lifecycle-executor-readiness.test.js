...
async function writeApprovalForDryRun(dir, configPath, operation = 'install', overrides = {}) {
  ...
  await writeFile(approvalPath, JSON.stringify({
    ...
    reason: 'V0.98 executor readiness approval reason must not leak',
    acknowledgements: ['operator accepts executor readiness gate'],
...
function assertNoSensitiveOutput(output, ...paths) {
  assert.doesNotMatch(output, /operator@example|executor readiness approval|operator accepts|sha256:|localhost|Documents|token|secret|password|approval\.json|linke-config|\/Users\/ah|Authorization/i);
...
describe('agent supervisor-lifecycle-executor-readiness', () => {
  it('prints blocked executor readiness without approval records and does not create approval storage', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'linke-executor-readiness-cli-empty-'));
    const storeDir = join(dir, 'store');
    try {
      const configPath = await writeTempConfig(dir);

      const { stdout } = await runAgent([
        'supervisor-lifecycle-executor-readiness',
        '--config', configPath,
        '--operation', 'install',
        '--data-dir', storeDir,
      ]);
      ...
      assert.strictEqual(report.command, 'supervisor-lifecycle-executor-readiness');
      assert.strictEqual(report.operation, 'install');
      assert.strictEqual(report.state, 'blocked');
      assert.strictEqual(report.executorState, 'blocked');
      assert.strictEqual(report.executorReady, false);
      assert.strictEqual(report.approvalRecordReady, false);
      assert.ok(report.blockers.includes('approval-record-gate-not-ready'));
      assert.ok(report.blockers.includes('approval-record-missing'));
      assert.ok(report.blockers.includes('executor-not-implemented-for-action:render-launch-agent-plist'));
      assert.ok(report.blockers.includes('executor-not-implemented-for-action:write-launch-agent-plist'));
      assert.ok(report.blockers.includes('executor-not-implemented-for-action:load-launch-agent'));
      assert.deepStrictEqual(report.executorBlockers.map((entry) => entry.actionId), [
  ...
  it('keeps executor readiness blocked after a persisted sanitized approval record is ready', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'linke-executor-readiness-cli-ready-'));
    const storeDir = join(dir, 'store');
    try {
      const configPath = await writeTempConfig(dir);
      ...
      assert.deepStrictEqual(report.nextBlockers, report.blockers);
      assert.deepStrictEqual(report.executorBlockers.map((entry) => entry.actionId), [
        'capture-current-state',
        'restore-previous-plist',
        'restart-previous-supervisor',
      ]);
      assert.strictEqual(report.gates.approvalRecordReady, true);
  ...
  it('--fail-on-blocked exits 2 after printing blocked executor readiness', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'linke-executor-readiness-cli-fail-'));
    const storeDir = join(dir, 'store');
    try {
      const configPath = await writeTempConfig(dir);
      ...
      assert.strictEqual(report.state, 'blocked');
      assert.strictEqual(report.executorReady, false);
      assert.ok(report.blockers.includes('executor-not-implemented-for-action:unload-launch-agent'));
      assertNoSensitiveOutput(`${error.stdout}${error.stderr}`, configPath, storeDir, dir);
    } finally {
      await rm(dir, { recursive: true, force: true });
    }
  });
  ...
  it('rejects unsupported write-like inputs with sanitized errors', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'linke-executor-readiness-cli-args-'));
    try {
      const configPath = await writeTempConfig(dir);
  ...
  it('sanitizes config and approval store read failures', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'linke-executor-readiness-cli-errors-'));
    try {
      ...
      assert.match(configError.stderr, /supervisor-lifecycle-executor-readiness failed; verify --config points to a readable valid Linke config/);
  ...
  it('help documents supervisor lifecycle executor readiness automation', async () => {
    const { stdout } = await runAgent(['--help']);
    assert.match(stdout, /supervisor-lifecycle-executor-readiness/);
