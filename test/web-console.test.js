...
describe('Web Console / API contract', () => {
  ...
  it('HTML contains supervisor lifecycle approval persistence preview panel with required data-testid hooks', async () => {
    ...
    const readinessButtonMatch = html.match(/<button[^>]*data-testid="supervisor-lifecycle-apply-readiness-button"[^>]*>([\s\S]*?)<\/button>/);
...
describe('buildSupervisorLifecycleApplyReadinessViewModel', () => {
  it('returns unknown state for missing payload', () => {
    const result = buildSupervisorLifecycleApplyReadinessViewModel(null);

    assert.strictEqual(result.statusKey, 'unknown');
    assert.strictEqual(result.statusText, '未检查');
    assert.strictEqual(result.approvalValidText, 'records:0');
    assert.strictEqual(result.persistenceText, 'readOnly:true / lifecycleApply:false');
    assert.deepStrictEqual(result.blockers, []);
    assert.deepStrictEqual(result.recordLines, []);
    assert.ok(result.safetyLines.includes('readOnly:true'));
    assert.ok(result.safetyLines.includes('lifecycleApplied:false'));
    assert.match(result.messageText, /apply readiness/i);
  });

  it('returns sanitized ready display fields without implying lifecycle apply execution', () => {
    const result = buildSupervisorLifecycleApplyReadinessViewModel({
      command: 'supervisor-lifecycle-apply-readiness',
      operation: 'install',
      state: 'blocked',
      approvalRecordState: 'ready',
      approvalRecordReady: true,
      blockers: [],
      nextBlockers: ['executor-implementation-missing'],
      approvalRecords: {
        readOnly: true,
        count: 1,
      ...
      gates: {
        ...
        applyFlag: true,
        envGate: true,
        approvalRecordPersisted: true,
        approvalRecordValid: true,
        approvalRecordOperationMatched: true,
    ...
    assert.strictEqual(result.statusKey, 'ready');
    assert.strictEqual(result.statusText, '批准记录就绪');
    assert.strictEqual(result.approvalValidText, 'approvalRecordReady:true');
    assert.strictEqual(result.persistenceText, 'readOnly:true / lifecycleApply:false');
    assert.ok(result.blockers.includes('next:executor-implementation-missing'));
    assert.ok(result.recordLines.includes('records:1'));
    assert.ok(result.recordLines.includes('operationMatches:1'));
    assert.ok(result.recordLines.includes('persistedMatches:1'));
    assert.ok(result.validationLines.includes('approvalRecordValid:true'));
    assert.ok(result.validationLines.includes('executorImplemented:false'));
    assert.ok(result.safetyLines.includes('readOnly:true'));
    assert.ok(result.safetyLines.includes('lifecycleApplied:false'));
    assert.match(result.messageText, /approval record gate ready/i);
    assert.match(result.messageText, /not lifecycle apply/i);
    assert.ok(!text.includes('operator@example'), 'must not expose approval identity values');
    assert.ok(!text.includes('secret reason'), 'must not expose approval reasons');
    assert.ok(!text.includes('sha256:secret'), 'must not expose hashes');
  ...
  it('returns sanitized blocked and error states', () => {
    const blocked = buildSupervisorLifecycleApplyReadinessViewModel({
      command: 'supervisor-lifecycle-apply-readiness',
      operation: 'rollback',
      state: 'blocked',
      approvalRecordReady: false,
      blockers: ['approval-record-missing'],
      nextBlockers: ['executor-implementation-missing'],
      approvalRecords: { readOnly: true, count: 0, operationMatchCount: 0, persistedMatchCount: 0 },
      gates: { applyFlag: true, envGate: true, approvalRecordValid: false },
      safety: { readOnly: true, lifecycleApplied: false },
    });
    const failed = buildSupervisorLifecycleApplyReadinessViewModel(null, 'Bearer token secret-value failed at /Users/ah/private');

    assert.strictEqual(blocked.statusKey, 'blocked');
    assert.strictEqual(blocked.statusText, '阻塞');
    assert.deepStrictEqual(blocked.blockers, ['approval-record-missing', 'next:executor-implementation-missing']);
    assert.strictEqual(failed.statusKey, 'error');
    assert.ok(!failed.messageText.includes('secret-value'), 'error message must be sanitized');
    assert.ok(!failed.messageText.includes('/Users/ah/private'), 'error path must be sanitized');
  });
...
describe('buildSupervisorLifecycleExecutorManifestReadinessViewModel', () => {
  it('returns unknown state for missing payload without implying execution readiness', () => {
    const result = buildSupervisorLifecycleExecutorManifestReadinessViewModel(null);

    assert.strictEqual(result.statusKey, 'unknown');
    assert.strictEqual(result.statusText, '未检查');
    assert.strictEqual(result.approvalValidText, 'manifestReady:false / executorReady:false');
    assert.strictEqual(result.persistenceText, 'readOnly:true / executorReady:false');
    assert.deepStrictEqual(result.blockers, []);
    assert.deepStrictEqual(result.recordLines, []);
    assert.ok(result.safetyLines.includes('readOnly:true'));
    assert.ok(result.safetyLines.includes('lifecycleApplied:false'));
    assert.match(result.messageText, /manifest readiness/i);
  });

  it('returns sanitized blocked executor manifest readiness fields without exposing sensitive payload details', () => {
    const result = buildSupervisorLifecycleExecutorManifestReadinessViewModel({
      ...
      manifestReady: false,
      executorReady: false,
      blockers: ['guarded-executor-runner-missing', 'executor-manifest-missing-for-action:/Users/ah/secret-path/render-launch-agent-plist'],
      manifestBlockers: ['executor-manifest-missing-for-action:/Users/ah/secret-path/render-launch-agent-plist'],
      ...
      actionManifests: [
        {
          ...
          wouldWrite: false,
          actionId: 'capture-current-state',
          implementationId: 'my-implementation',
    ...
    assert.ok(result.blockers.includes('executor-manifest-missing-for-action:[redacted-path]'));
    assert.ok(result.blockers.includes('manifest:executor-manifest-missing-for-action:[redacted-path]'));
    assert.ok(result.blockers.includes('next:guarded-executor-runner-missing'));
    assert.ok(result.requiredFields.includes('action:capture-current-state:impl:my-implementation:mode:guarded-host-action:wouldRun:false:wouldWrite:false'));
    assert.ok(result.validationLines.includes('lifecyclePlanValid:true'));
    assert.ok(result.validationLines.includes('manifestReady:false'));
    assert.ok(result.validationLines.includes('executorImplemented:false'));
    assert.ok(result.safetyLines.includes('readOnly:true'));
    assert.ok(result.safetyLines.includes('lifecycleApplied:false'));
    assert.ok(result.safetyLines.includes('launchctlCalled:false'));
    assert.match(result.messageText, /executor manifest readiness/i);
    assert.match(result.messageText, /fail-closed/i);
    assert.ok(!text.includes('/Users/ah/secret-path'), 'must not expose paths');
  });

  it('returns sanitized error state', () => {
    const failed = buildSupervisorLifecycleExecutorManifestReadinessViewModel(null, 'Bearer token secret-value failed at /Users/ah/private');
...
describe('DOM test: supervisor lifecycle approval persistence preview panel interactions', () => {
  ...
  it('does not request /api/supervisor-lifecycle-apply-readiness on initialization', async () => {
    let fetchCount = 0;
    const doc = buildMockDoc();
    const fetchImpl = async (url) => {
      if (String(url) === '/api/supervisor-lifecycle-apply-readiness') fetchCount++;
      return { ok: true, status: 200, json: async () => [] };
    };
  ...
  it('validates apply readiness config locally without parsing approval JSON or calling the API', async () => {
    const calls = [];
    const doc = buildMockDoc();
    initConsole(
      doc,
  ...
  it('does not request /api/supervisor-lifecycle-executor-manifest-readiness on initialization', async () => {
    let fetchCount = 0;
    const doc = buildMockDoc();
    const fetchImpl = async (url) => {
      if (String(url) === '/api/supervisor-lifecycle-executor-manifest-readiness') fetchCount++;
      return { ok: true, status: 200, json: async () => [] };
    };
  ...
  it('does not request /api/supervisor-lifecycle-guarded-runner-execution-preview on initialization', async () => {
    let fetchCount = 0;
    const doc = buildMockDoc();
    const fetchImpl = async (url) => {
      if (String(url) === '/api/supervisor-lifecycle-guarded-runner-execution-preview') fetchCount++;
      return { ok: true, status: 200, json: async () => [] };
    };
  ...
  it('validates guarded runner execution preview config, manifest, and runner binding locally without calling the API', async () => {
    const calls = [];
    const doc = buildMockDoc();
    initConsole(
      doc,
    ...
    assert.ok(!calls.some((url) => String(url) === '/api/supervisor-lifecycle-guarded-runner-execution-preview'));
    assert.match(doc.getElementById('supervisor-lifecycle-approval-preview-result').textContent, /配置 JSON 不能为空/);
  ...
  it('builds a fail-closed sanitized guarded runner execution preview view model', () => {
    const viewModel = buildSupervisorLifecycleGuardedRunnerExecutionPreviewViewModel({
      ...
      actionPreviews: [
        ...
        {
          actionId: 'render-rollback-anchor',
          implementationId: 'safe-renderer sk-abc ghp_ab xoxb-abc',
          mode: 'ruby -e "puts 1" ghs_abc123def456',
          runnerKind: 'perl -e "print 1"',
        },
      ],
      gates: {
        lifecyclePlanValid: true,
    ...
    assert.strictEqual(viewModel.statusText, '执行预览阻塞');
    assert.strictEqual(viewModel.approvalValidText, 'executionReady:false / executorReady:false');
    assert.strictEqual(viewModel.persistenceText, 'readOnly:true / wouldExecute:false');
    ...
    assert.match(text, /guarded-runner-execution-preview-only/);
    ...
    assert.match(text, /executionPreviewOnly:true/);
    ...
    assert.ok(!text.includes('QWxhZGRpbjpvcGVuIHNlc2FtZQ=='), 'must not expose base64-like secrets');
    assert.ok(!text.includes('SECRET_XYZ'), 'must not expose token values');
    assert.ok(!text.includes('sk-abc'), 'must not expose very short sk-prefixed token values');
    assert.ok(!text.includes('ghp_ab'), 'must not expose very short GitHub-prefixed token values');
    assert.ok(!text.includes('xoxb-abc'), 'must not expose very short Slack-prefixed token values');
    assert.ok(!text.includes('sk-abc123def456'), 'must not expose short sk-prefixed token values');
    assert.ok(!text.includes('ghp_abc123def456'), 'must not expose short ghp-prefixed token values');
    assert.ok(!text.includes('ghs_abc123def456'), 'must not expose short ghs-prefixed token values');
    ...
    assert.ok(!text.includes('command=/bin/sh'), 'must not expose command fields');
    ...
    assert.doesNotMatch(text, /\bpython3\b/i, 'must not expose python3 command-like runner metadata');
  ...
  it('builds a fail-closed sanitized guarded runner execution gate view model', () => {
    const viewModel = buildSupervisorLifecycleGuardedRunnerExecutionGateViewModel({
      ...
      runnerWiringContract: {
        ...
        requiredContracts: [
          {
            // Strict ready contract only (status + null blocker + evidenceCode); opaque
            // evidence/command strings must not leak into the Web line.
            id: 'execution-policy',
            status: 'ready',
          ...
          {
            // Malicious ready injection for a non-execution-policy contract — must stay blocked.
            id: 'runner-registry',
            status: 'ready',
            requiredForExecution: false,
            evidence: '/Users/ah/secret-path token=SECRET_XYZ approval reason',
    ...
    assert.doesNotMatch(text, /policyDecision:state:authorized|executionEligible:true|wouldExecute:true/);
  });
  ...
  it('keeps guarded runner execution gate unknown and error validation fail-closed for execution policy and operator recovery', () => {
    ...
    for (const vm of [unknown, error]) {
      assert.deepStrictEqual(vm.requiredFields, []);
  ...
  function fullCanonicalRunnerRegistryReadyPayload(overrides = {}) {
    ...
    const {
      ...
      capabilityInjectionReadiness: capabilityInjectionReadinessOverrides,
      ...topLevelOverrides
    } = overrides;
    return {
      command: 'supervisor-lifecycle-guarded-runner-execution-gate',
  ...
  it('W1: full canonical ready payload renders ready wiring/registry/adapter/anchor/audit/recovery + authorized policy + blocked sentinel', () => {
    ...
    // V1.32 shall: realRenderCapability fixed line
    assert.match(
      text,
      /^realRenderCapability:state:ready:realRenderReady:true:hostSideEffectOccurred:false:realCapabilityImplementationsReady:false:realRunnerWiringReady:false:blocker:none$/m,
    );
    // V1.33 shall: realStatusCapability fixed non-live line (no statusResult)
    assert.match(
      text,
    ...
    assert.ok(viewModel.validationLines.includes('hostSideEffectOccurred:false'));
  ...
  it('V1.32 W-realRender: shall line + malicious elevation never exposes content or elevates global real', () => {
    const ready = buildSupervisorLifecycleGuardedRunnerExecutionGateViewModel(
      fullCanonicalRunnerRegistryReadyPayload(),
    );
    const readyText = [...ready.requiredFields, ...ready.validationLines].join('\n');
    assert.match(
      readyText,
      /^realRenderCapability:state:ready:realRenderReady:true:hostSideEffectOccurred:false:realCapabilityImplementationsReady:false:realRunnerWiringReady:false:blocker:none$/m,
    );
    ...
    assert.ok(ready.validationLines.includes('realRenderCapabilityImplementationReady:true'));
    assert.ok(ready.validationLines.includes('realCapabilityImplementationsReady:false'));
    assert.ok(ready.validationLines.includes('hostSideEffectOccurred:false'));
    assert.doesNotMatch(readyText, /contentSha256|9883ab81|__LINKE_REDACTED/);
    ...
    const text = [...viewModel.requiredFields, ...viewModel.validationLines].join('\n');
    // global real still shown false on the realRender shall line
    assert.match(
      text,
      /^realRenderCapability:state:not-ready:realRenderReady:false:hostSideEffectOccurred:false:realCapabilityImplementationsReady:false:realRunnerWiringReady:false:blocker:real-render-capability-not-ready$/m,
    );
    assert.ok(viewModel.validationLines.includes('realCapabilityImplementationsReady:false'));
    assert.ok(viewModel.validationLines.includes('executionEligible:false'));
    assert.ok(viewModel.validationLines.includes('realRunnerWiringReady:false'));
    assert.doesNotMatch(text, /realCapabilityImplementationsReady:true|executionEligible:true|realRunnerWiringReady:true/);
  });

  it('V1.33 W-realStatus: shall non-live line + no statusResult + elevation never elevates global real', () => {
    const ready = buildSupervisorLifecycleGuardedRunnerExecutionGateViewModel(
      fullCanonicalRunnerRegistryReadyPayload(),
    );
    const readyText = [...ready.requiredFields, ...ready.validationLines].join('\n');
    ...
    assert.ok(ready.validationLines.includes('realStatusCapabilityImplementationReady:true'));
    assert.ok(ready.validationLines.includes('hostObservationOccurred:false'));
    assert.ok(ready.validationLines.includes('hostSideEffectOccurred:false'));
    assert.ok(ready.validationLines.includes('hostMutationOccurred:false'));
    assert.ok(ready.validationLines.includes('realCapabilityImplementationsReady:false'));
    assert.doesNotMatch(readyText, /statusResult|contentSha256|\/Users\/|LaunchAgents/);
    ...
    const text = [...viewModel.requiredFields, ...viewModel.validationLines].join('\n');
    // elevation of global/observation facts must not mark realStatus ready (live elevation rejected)
    assert.match(
      text,
      /^realStatusCapability:state:not-ready:realStatusReady:false:hostObservationOccurred:false:hostSideEffectOccurred:false:hostMutationOccurred:false:realCapabilityImplementationsReady:false:realRunnerWiringReady:false:blocker:real-status-capability-not-ready$/m,
    );
    assert.ok(viewModel.validationLines.includes('realCapabilityImplementationsReady:false'));
    assert.ok(viewModel.validationLines.includes('hostObservationOccurred:false'));
    assert.ok(viewModel.validationLines.includes('executionEligible:false'));
  ...
  it('W12b: C/A/G ready + D.wouldWriteAnchor true blocks anchor wiring/anchor/validation', () => {
    ...
    assert.doesNotMatch(text, /wiringContract:rollback-anchor:status:ready/);
    assert.doesNotMatch(text, /rollbackAnchor:code-owned-rollback-anchor:state:ready/);
  });

  it('W12c: malicious secret anchorKind does not leak and stays fixed blocked', () => {
    const OPAQUE_UNSAFE_FIELD = 'OPAQUE_UNSAFE_FIELD';
    const base = fullCanonicalRunnerRegistryReadyPayload();
    const viewModel = buildSupervisorLifecycleGuardedRunnerExecutionGateViewModel({
      ...base,
      anchorDecision: undefined,
      gates: {
        ...base.gates,
        rollbackAnchorReady: false,
      },
      runnerWiringContract: {
        ...
        rollbackAnchorReadiness: {
          ...
          realRollbackAnchorImplementationReady: false,
          anchorEntries: [{
            anchorKind: OPAQUE_UNSAFE_FIELD,
            wouldWriteAnchor: true,
            wouldRestore: true,
          }],
        },
    ...
    assert.doesNotMatch(text, new RegExp(OPAQUE_UNSAFE_FIELD));
    assert.doesNotMatch(text, /disabled-rollback-anchor-stub/);
    assert.ok(viewModel.validationLines.includes('rollbackAnchorReady:false'));
  });
  ...
  it('maps ready contract blockerCode null to UI sentinel none without throw', () => {
    const readyPayload = {
      ...
      policyDecision: {
        ...
        authorized: false,
        wouldAuthorizeExecution: false,
        primaryBlocker: 'runner-registry-not-ready',
        blockers: ['runner-registry-not-ready'],
      },
      runnerWiringContract: {
        requiredContracts: [{
          id: 'execution-policy',
    ...
    assert.match(text, /executionPolicy:fail-closed-execution-policy:state:ready:realImplementationReady:true:wouldAuthorizeExecution:false:blocker:none/);
    assert.match(text, /policyDecision:state:denied:authorized:false:wouldAuthorizeExecution:false:primaryBlocker:runner-registry-not-ready/);
  ...
  it('keeps host-mutation-adapter wiring contract blocked when payload injects status ready', () => {
    const viewModel = buildSupervisorLifecycleGuardedRunnerExecutionGateViewModel({
      command: 'supervisor-lifecycle-guarded-runner-execution-gate',
      state: 'ready',
      executionGateState: 'completed',
        ...
        requiredContracts: [
          {
            ...
            status: 'ready',
            requiredForExecution: false,
            evidence: 'UNSAFE_SECRET_MATERIAL launchctl /Users/ah/.ssh/id_rsa',
            evidenceCode: 'host-mutation-adapter-ready',
            blockerCode: null,
            command: 'launchctl load /Users/ah/Library/LaunchAgents/linke.plist',
          },
    ...
    assert.doesNotMatch(text, /wiringContract:host-mutation-adapter:status:ready/);
    assert.doesNotMatch(text, /host-mutation-adapter-ready/);
    assert.ok(!text.includes('UNSAFE_SECRET_MATERIAL'), 'must not leak injected evidence');
    assert.ok(!text.includes('/Users/ah'), 'must not expose paths from ready injection');
    assert.ok(!text.includes('launchctl'), 'must not expose command-like contract fields');
    assert.match(text, /hostMutationAdapterReady:false/);
  });
  ...
  it('handles malicious attempt audit payload without leaking or execution permissions', () => {
    const viewModel = buildSupervisorLifecycleGuardedRunnerExecutionGateViewModel({
      command: 'supervisor-lifecycle-guarded-runner-execution-gate',
      state: 'ready',
      executionGateState: 'completed',
  ...
  it('manually checks apply readiness and renders sanitized read-only gate status', async () => {
    const calls = [];
    const doc = buildMockDoc();
    initConsole(
      doc,
      async (url, options) => {
        calls.push({ url, options });
        if (String(url) === '/api/supervisor-lifecycle-apply-readiness') {
          return {
            ok: true,
            status: 200,
            json: async () => ({
    ...
    doc.getElementById('supervisor-lifecycle-approval-preview-config').value = JSON.stringify({
      serverUrl: 'http://secret.localhost:3000',
      deviceId: 'web-lifecycle-apply-readiness',
      backupJobs: [{ name: 'documents', sourcePath: '/tmp/linke-documents' }],
    });
    doc.getElementById('supervisor-lifecycle-approval-preview-approval').value = '{ invalid approval ignored';
    doc.getElementById('supervisor-lifecycle-apply-readiness-button')._listeners.click();
    ...
    const apiCall = calls.find((call) => String(call.url) === '/api/supervisor-lifecycle-apply-readiness');
    assert.ok(apiCall, 'must call apply readiness API');
    assert.strictEqual(apiCall.options.method, 'POST');
    const requestBody = JSON.parse(apiCall.options.body);
    assert.strictEqual(requestBody.operation, 'install');
    assert.strictEqual(requestBody.config.deviceId, 'web-lifecycle-apply-readiness');
    assert.ok(!Object.hasOwn(requestBody, 'approval'), 'readiness must not submit approval JSON');
    assert.strictEqual(doc.getElementById('supervisor-lifecycle-approval-preview-status').textContent, '批准记录就绪');
    assert.strictEqual(doc.getElementById('supervisor-lifecycle-approval-preview-valid').textContent, 'approvalRecordReady:true');
    assert.strictEqual(doc.getElementById('supervisor-lifecycle-approval-preview-persist').textContent, 'readOnly:true / lifecycleApply:false');

    const resultText = doc.getElementById('supervisor-lifecycle-approval-preview-result').textContent;
    assert.match(resultText, /approval record gate ready/i);
    assert.match(resultText, /next:executor-implementation-missing/);
    assert.match(resultText, /records:1/);
    assert.match(resultText, /approvalRecordValid:true/);
    assert.match(resultText, /lifecycleApplied:false/);
    assert.ok(!resultText.includes('operator@example'), 'must not render approval identity value');
    assert.ok(!resultText.includes('secret reason'), 'must not render approval reason');
    assert.ok(!resultText.includes('sha256:secret'), 'must not render hash values');
  ...
  it('does not start a second apply readiness request while one is in flight', async () => {
    let fetchCount = 0;
    let resolveRequest;
    const doc = buildMockDoc();
    initConsole(
        ...
        if (String(url) === '/api/supervisor-lifecycle-apply-readiness') {
          ...
          return {
            ...
            json: async () => ({
              ...
              nextBlockers: ['executor-implementation-missing'],
              approvalRecords: { readOnly: true, count: 0, operationMatchCount: 0, persistedMatchCount: 0 },
              gates: { approvalRecordValid: false },
              safety: { readOnly: true, lifecycleApplied: false },
            }),
          };
        }
    ...
    const readinessBtn = doc.getElementById('supervisor-lifecycle-apply-readiness-button');
  ...
  it('manually checks executor manifest readiness and renders sanitized read-only gate status', async () => {
    const calls = [];
    const doc = buildMockDoc();
    initConsole(
      doc,
      async (url, options) => {
        calls.push({ url, options });
        if (String(url) === '/api/supervisor-lifecycle-executor-manifest-readiness') {
          return {
            ok: true,
            status: 200,
            json: async () => ({
    ...
    doc.getElementById('supervisor-lifecycle-approval-preview-config').value = JSON.stringify({
      serverUrl: 'http://localhost:3000',
      deviceId: 'web-lifecycle-executor-manifest-readiness',
      backupJobs: [{ name: 'documents', sourcePath: '/tmp/source' }],
    });
    ...
    const manifestBtn = doc.getElementById('supervisor-lifecycle-executor-manifest-readiness-button');
    manifestBtn._listeners.click();
    await new Promise((resolve) => setTimeout(resolve, 30));

    const apiCall = calls.find((call) => String(call.url) === '/api/supervisor-lifecycle-executor-manifest-readiness');
    assert.ok(apiCall, 'must call executor manifest readiness API');
    assert.strictEqual(apiCall.options.method, 'POST');

    const requestBody = JSON.parse(apiCall.options.body);
    assert.strictEqual(requestBody.operation, 'install');
    assert.strictEqual(requestBody.config.deviceId, 'web-lifecycle-executor-manifest-readiness');
    ...
    assert.strictEqual(doc.getElementById('supervisor-lifecycle-approval-preview-status').textContent, '执行器未就绪');
    assert.strictEqual(doc.getElementById('supervisor-lifecycle-approval-preview-valid').textContent, 'manifestReady:false / executorReady:false');
    assert.strictEqual(doc.getElementById('supervisor-lifecycle-approval-preview-persist').textContent, 'readOnly:true / executorReady:false');

    const resultText = doc.getElementById('supervisor-lifecycle-approval-preview-result').textContent;
    assert.match(resultText, /guarded-executor-runner-missing/);
    assert.match(resultText, /executor-manifest-missing-for-action:\[redacted-path\]/);
    assert.match(resultText, /action:capture-current-state:impl:my-implementation:mode:guarded-host-action:wouldRun:false:wouldWrite:false/);
    assert.match(resultText, /lifecyclePlanValid:true/);
    assert.match(resultText, /manifestReady:false/);
    assert.match(resultText, /executorImplemented:false/);
    assert.match(resultText, /readOnly:true/);
    assert.match(resultText, /lifecycleApplied:false/);
  ...
  it('does not start a second executor manifest readiness request while one is in flight', async () => {
    let fetchCount = 0;
    let resolveRequest;
    const doc = buildMockDoc();
    initConsole(
    ...
    doc.getElementById('supervisor-lifecycle-approval-preview-config').value = JSON.stringify({
      serverUrl: 'http://localhost:3000',
      deviceId: 'web-lifecycle-executor-manifest-readiness-inflight',
      backupJobs: [{ name: 'documents', sourcePath: '/tmp/source' }],
    });
    doc.getElementById('supervisor-lifecycle-executor-manifest-readiness-manifest').value = JSON.stringify({
      kind: 'supervisor-lifecycle-executor-manifest',
  ...
  it('requests guarded runner execution preview manually with inline payload and renders sanitized blocked fields', async () => {
    const calls = [];
    const doc = buildMockDoc();
    initConsole(
      doc,
    ...
    doc.getElementById('supervisor-lifecycle-approval-preview-config').value = JSON.stringify({
      serverUrl: 'http://localhost:3000',
      deviceId: 'web-lifecycle-guarded-runner-execution-preview',
      backupJobs: [{ name: 'documents', sourcePath: '/tmp/source' }],
    });
    doc.getElementById('supervisor-lifecycle-approval-preview-approval').value = JSON.stringify({
      approvedBy: 'operator@example.invalid',
    ...
    const resultText = doc.getElementById('supervisor-lifecycle-approval-preview-result').textContent;
    assert.match(resultText, /guarded-runner-execution-preview-only/);
    assert.match(resultText, /real-guarded-runner-execution-wiring-missing/);
    assert.match(resultText, /wouldExecute:false/);
    assert.match(resultText, /wouldRun:false/);
    assert.match(resultText, /wouldWrite:false/);
    assert.match(resultText, /executionPreviewOnly:true/);
    assert.match(resultText, /executorReady:false/);
    assert.match(resultText, /readOnly:true/);
    assert.ok(!resultText.includes('/Users/ah'), 'must not expose paths');
    assert.ok(!resultText.includes('SECRET_XYZ'), 'must not expose token values');
    assert.ok(!resultText.includes('sk-abc'), 'must not expose very short sk-prefixed token values');
    assert.ok(!resultText.includes('ghp_ab'), 'must not expose very short GitHub-prefixed token values');
    assert.ok(!resultText.includes('xoxb-abc'), 'must not expose very short Slack-prefixed token values');
    ...
    assert.doesNotMatch(resultText, /\bpython\b/i, 'must not expose python command-like runner metadata');
  ...
  it('does not start a second guarded runner execution preview request while one is in flight', async () => {
    let fetchCount = 0;
    let resolveRequest;
    const doc = buildMockDoc();
    initConsole(
    ...
    doc.getElementById('supervisor-lifecycle-approval-preview-config').value = JSON.stringify({
      serverUrl: 'http://localhost:3000',
      deviceId: 'web-lifecycle-guarded-runner-execution-preview-inflight',
      backupJobs: [{ name: 'documents', sourcePath: '/tmp/source' }],
    });
    doc.getElementById('supervisor-lifecycle-executor-manifest-readiness-manifest').value = JSON.stringify({
      kind: 'supervisor-lifecycle-executor-manifest',
    ...
    assert.strictEqual(readinessButton.disabled, true, 'execution preview in-flight must disable guarded runner readiness button');
  ...
  it('does not start guarded runner execution preview while guarded runner readiness is in flight', async () => {
    ...
    doc.getElementById('supervisor-lifecycle-approval-preview-config').value = JSON.stringify({
      serverUrl: 'http://localhost:3000',
      deviceId: 'web-lifecycle-guarded-runner-cross-inflight',
      backupJobs: [{ name: 'documents', sourcePath: '/tmp/source' }],
    });
    doc.getElementById('supervisor-lifecycle-executor-manifest-readiness-manifest').value = JSON.stringify({
      kind: 'supervisor-lifecycle-executor-manifest',
    ...
    const guardedRunnerButton = doc.getElementById('supervisor-lifecycle-guarded-runner-readiness-button');
    const previewButton = doc.getElementById('supervisor-lifecycle-guarded-runner-execution-preview-button');
    guardedRunnerButton._listeners.click();
    previewButton._listeners.click();
