/**
 * G0c auto / real-LAN acceptance — common secret-safe primitives.
 * Import has zero side effects. Real hardware only after exact REAL gate.
 * Auto production-boundary uses 0600 private bundle files (never secret-bearing env).
 * Auto harness complete ≠ real-LAN complete; ≠ Gold.
 */
...
export const REAL_GATE_ENV = 'LINKE_REAL_G0C_RESTORE_ACCEPTANCE';
export const REAL_GATE_VALUE = 'enabled';

/** Exact auto production-boundary gate (loopback only; not real-LAN evidence). */
export const AUTO_GATE_ENV = 'LINKE_G0C_AUTO_ACCEPTANCE';
export const AUTO_GATE_VALUE = 'enabled';

export const SCHEMA_VERSION = 1;
export const PURPOSE = 'linke-g0c-auto-acceptance';
export const RUN_MARKER_FILE = '.linke-g0c-auto-run.json';
export const AUTO_BUNDLE_FILE = 'auto-endpoint-bundle.json';
export const READY_MARKER_FILE = '.g0c-auto-ready.json';
export const STOP_MARKER_FILE = '.g0c-auto-stop';
/** Endpoint→controller backpressure slot release request (private under runDir). */
export const BACKPRESSURE_RELEASE_REQUEST_FILE = '.g0c-backpressure-release-request';
/** Controller→endpoint backpressure slot release ACK. */
export const BACKPRESSURE_RELEASE_ACK_FILE = '.g0c-backpressure-release-ack';
/** Endpoint-local disconnect mid-transfer marker (under endpoint data dir). */
export const DISCONNECT_MID_MARKER_FILE = '.g0c-disconnect-mid.json';
...
/** Fixed auto harness scenarios (production boundary via child_process endpoints). */
...
/**
 * Exact real-LAN gate: LINKE_REAL_G0C_RESTORE_ACCEPTANCE === 'enabled'.
...
/**
 * Exact auto gate: LINKE_G0C_AUTO_ACCEPTANCE === 'enabled'.
 * @param {NodeJS.ProcessEnv | Record<string, string | undefined>} [env]
 * @returns {true}
 */
...
export async function atomicWritePrivateJson(
  ...
  const tempName = `.g0c-${randomBytesImpl(16).toString('hex')}.new`;
...
export function validateAutoEndpointBundle(value) {
  ...
  assertExactKeys(value, [
    ...
    'deviceB',
    'restoreRootRelative',
    'endpointDataDirRelativeA',
    'endpointDataDirRelativeB',
  ]);
  if (value.schemaVersion !== SCHEMA_VERSION) throw requestInvalidError();
  if (value.purpose !== PURPOSE) throw requestInvalidError();
  if (value.kind !== 'auto-endpoint') throw requestInvalidError();
  const scenario = assertAutoScenario(value.scenario);
  ...
  const restoreRootRelative = assertNonEmptyString(value.restoreRootRelative);
  const endpointDataDirRelativeA = assertNonEmptyString(value.endpointDataDirRelativeA);
  const endpointDataDirRelativeB = assertNonEmptyString(value.endpointDataDirRelativeB);
  for (const rel of [restoreRootRelative, endpointDataDirRelativeA, endpointDataDirRelativeB]) {
...
export function serializeSanitizedResult(result) {
  ...
  // Fixed key order for stable single-line stdout.
  /** @type {Record<string, unknown>} */
  const ordered = {};
  for (const key of SANITIZED_RESULT_KEYS) {
