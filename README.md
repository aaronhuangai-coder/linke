...
> **当前版本：V0.81** — 单机 localhost 原型阶段，新增 `agent.js supervisor-install-dry-run` full JSON 内的 sanitized `installCommandPreview` 非 runnable 安装命令预览，固定 `installCommandPreview.state:"blocked"`，所有预览 action 均为 `wouldRun:false`、`wouldWrite:false`、`sensitiveValuesReturned:false`；继续具备 `agent.js supervisor-install-dry-run --readiness-summary / --fail-on-blocked` 本地 sanitized supervisor 安装就绪门禁、完整 dry-run plan 的 `readinessSummary.state:"blocked"`、summary-only blocked 摘要与 blocked 退出码 2、Web Console `supervisor-status-panel` 手动只读面板、`agent.js supervisor-status` 与 GET `/api/supervisor-status` 只读 supervisor 状态基础、`agent.js auth-status` 只读认证状态 CLI、`agent.js gold-readiness` 只读 Gold blocker 自动化门禁、Web Console audit-log 手动只读面板、`agent.js audit-log` 只读 CLI、Web Console hardening-status 手动只读面板、`agent.js hardening-status` 只读 CLI、GET `/api/hardening-status` 硬化状态只读接口与 buildHardeningStatusResponse 响应构建器、不泄露 token/restoreRoot 原值、nas-dry-run --fail-on-blocked 退出门禁、NAS CLI readiness-summary 摘要输出、NAS 执行就绪性摘要、目标就绪状态与 blocker codes 网页控制台只读展示、credentialRefConfigured sanitized dry-run 输出与网页展示、不泄露 credentialRef 原值、15 个 credential-like 字段精确拒绝与执行门禁网页展示、credentialRef 非密钥引用基础、固定 executionGate 远程执行阻塞、可选 Bearer token API 认证骨架、read/write token 授权基础、共享写入路由注册表、GET `/api/auth-status` 认证状态只读接口、Web Console 内存态 token UX、请求体上限、500 错误脱敏、可选恢复目标 root guard、恢复目标 symlink 写入防护、本地 audit log foundation 与 GET `/api/audit-log` sanitized 查询、可选 API rate-limit foundation 与可选 audit retention foundation，但尚未具备真实 NAS 传输、完整生产级鉴权、分布式限流、生产级审计、真实 supervisor 托管生命周期、真实安装、launchd install/start、watchdog 或 recovery supervisor 能力。
...
- **Gold readiness scorecard** — Web Console 提供 `gold-readiness-panel`，手动 GET `/api/gold-readiness` 展示静态 code-owned capability/blocker scorecard；`agent.js gold-readiness` 可通过同一只读端点输出 sanitized JSON，`--fail-on-blocked` 在 Gold blocked 时退出码 2；V0.81 继续将 `nas-dry-run` (含 CLI fail-on-blocked 自动化门禁)、`security-auth`、`automation-installation` 与 `production-hardening` (含 Agent supervisor-install-dry-run CLI、buildSupervisorInstallDryRunPlan、buildSupervisorInstallReadinessSummary、buildSupervisorInstallCommandPreview、supervisor-install-dry-run --readiness-summary、supervisor-install-dry-run --fail-on-blocked、readinessSummary.state:"blocked"、installCommandPreview.state:"blocked"、installCommandPreview.actions:wouldRun:false、Web Console supervisor-status 面板、Agent supervisor-status CLI、GET `/api/supervisor-status`、buildSupervisorStatusResponse、buildSupervisorStatusViewModel、Agent auth-status CLI、Agent Gold readiness CLI、Web Console audit-log 面板、Agent audit-log CLI、Web Console hardening-status 面板、`agent.js hardening-status` 只读 CLI、GET `/api/hardening-status` 硬化状态只读接口与 buildHardeningStatusResponse 响应构建器) 标为 partial，`real-nas-remote-backup` 仍为 Gold blocker
...
- **Supervisor install dry-run CLI** — `agent.js supervisor-install-dry-run --config <file>` 读取本地配置后只输出 sanitized JSON 安装计划，由 `buildSupervisorInstallDryRunPlan` 构造；V0.80 增加 `buildSupervisorInstallReadinessSummary`、`readinessSummary.state:"blocked"`、`--readiness-summary` 与 `--fail-on-blocked`，summary-only 模式只输出就绪摘要，`--fail-on-blocked` 在 blocked 时打印 JSON 后退出码为 2；V0.81 增加 `buildSupervisorInstallCommandPreview` 与 full JSON 内的 `installCommandPreview`，该 preview 固定 blocked、不可执行，所有 action 均为 `wouldRun:false`、`wouldWrite:false`、`sensitiveValuesReturned:false`；该命令固定 `wouldInstall:false`、`wouldStart:false`、`launchctlCalled:false`、`processListRead:false`、`launchdFileWritten:false`、`metadataWritten:false`、`nasConnected:false`、`backupTriggered:false`、`restoreTriggered:false`、`remoteCommandExecuted:false`，不调用 launchctl、不读取进程列表、不安装或启动 launchd、不写 LaunchAgents/plist、不写 metadata、不连接 NAS、不触发备份/恢复或远程命令；不回显 config path、sourcePath、serverUrl、NAS endpoint、credentialRef、token、Authorization 或 env 值；`--output` 不支持，若需要 plist 预览应继续使用单独的 `launchd-dry-run`
...
- **Gold 边界**：supervisor-install-dry-run、`buildSupervisorInstallReadinessSummary`、`buildSupervisorInstallCommandPreview`、`--readiness-summary`、`--fail-on-blocked`、`readinessSummary.state:"blocked"` 与 `installCommandPreview.state:"blocked"` 只是 `automation-installation` 与 `production-hardening` 的 partial evidence；真实 installer、launchd install/start、watchdog、监控、recovery supervisor、部署硬化和生产安全边界仍未实现，Gold 发布仍 blocked。
...
- **Gold blockers**：V0.81 基于 V0.80 blocked readiness gate、V0.79 Agent `supervisor-install-dry-run` CLI、`buildSupervisorInstallDryRunPlan` 与 `test/agent-supervisor-install-dry-run.test.js` sanitized 安装计划 dry-run，继续保留 V0.78 Web Console `supervisor-status-panel`、V0.77 GET `/api/supervisor-status` 与 Agent supervisor-status、V0.76 Agent auth-status、V0.75 Agent Gold readiness、V0.74 Web Console audit-log、V0.73 Agent CLI audit-log、V0.72 Web Console hardening-status、V0.71 Agent `hardening-status`、V0.70 GET /api/hardening-status 和 V0.69 CLI fail-on-blocked / readiness-summary NAS 自动化门禁证据；新增 `buildSupervisorInstallCommandPreview`、`installCommandPreview.state:"blocked"` 与 `installCommandPreview.actions:wouldRun:false` 证据。`nas-dry-run`、`security-auth`、`automation-installation` 与 `production-hardening` 继续为 partial，`real-nas-remote-backup` 仍为 blocked。
...
- **NAS、认证和生产硬化仍是 partial**：V0.81 基于 V0.80 blocked readiness gate、V0.79 Agent `supervisor-install-dry-run` sanitized 安装计划 dry-run、V0.78 Web Console `supervisor-status-panel`、V0.77 GET `/api/supervisor-status` 与 Agent supervisor-status、V0.76 Agent auth-status、V0.75 Agent Gold readiness、V0.74 Web Console audit-log、V0.73 Agent CLI audit-log、V0.72 Web Console hardening-status、V0.71 Agent CLI `hardening-status`、V0.70 GET /api/hardening-status，以及 V0.69 CLI fail-on-blocked 自动化门禁、readiness-summary 摘要输出和 NAS dry-run 执行就绪性摘要；新增 `buildSupervisorInstallCommandPreview`、`installCommandPreview.state:"blocked"` 与 `installCommandPreview.actions:wouldRun:false`。这些只提供 sanitized 可见性和 blocker gate，并继续保留 `tokenValuesReturned:false`、`restoreRootValueReturned:false`、`auditPathReturned:false`、`environmentValuesReturned:false`、`supervisor.state:not_configured`、`supervisorInstalled:false` 等脱敏证据；不代表生产硬化完成；没有真实 NAS transport、用户、角色权限、生产级审计、secret management、token 轮换、distributed rate limiting、监控、托管 supervisor 或 recovery supervisor。
