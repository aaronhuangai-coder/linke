...
> **当前版本：V0.98** — 单机 localhost 原型阶段，新增 Agent CLI 只读 `supervisor-lifecycle-executor-readiness` 命令，读取 data-dir 下的 approval records 后返回带 action-driven executor blockers 的 read-only fail-closed 执行器就绪性评估；top-level `state:"blocked"` 且 `executorReady:false`，`safety` 中 `readOnly:true` 且不改变主机/launchd/文件系统状态；配置文件错误报 sanitized 'supervisor-lifecycle-executor-readiness failed; verify --config points to a readable valid Linke config'，approval store 读取错误报 sanitized 'failed to read supervisor lifecycle approval records'，不泄露 approval 原文、hash、路径、URL、token 或 secret，不写 approval store、不执行 lifecycle apply、不调用 launchctl，Gold 依旧 blocked
