...
| V0.65 | 当前版本 | Web NAS execution gate display：Web Console 支持展示 NAS 凭证配置状态与 executionGate 远程执行阻塞，不启用真实 NAS 传输，Gold 依旧 blocked |
...
- **NAS dry-run** — API、CLI 与 Web Console 可验证 Synology / Ugreen 目标配置并输出计划；V0.65 支持 `credentialRef` 非密钥引用基础、`credentialRefConfigured` 网页展示与 `executionGate` 门禁展示，不连接 NAS、不写远端、不保存凭证、不回显引用原值
...
- **Gold readiness scorecard** — Web Console 提供 `gold-readiness-panel`，手动 GET `/api/gold-readiness` 展示静态 code-owned capability/blocker scorecard；V0.65 将 `nas-dry-run`、`security-auth` 与 `production-hardening` 标为 partial，`real-nas-remote-backup` 仍为 Gold blocker
...
- **非密钥引用**：`credentialRef` 只是引用名，不是用户名、密码、token、API key 或 secret；`username`、`password`、`token`、`apiKey`、`secret`、`accessKey`、`refreshToken` 等字段仍会被拒绝。
...
- 返回的健康数据包含：`status`（当 `dataDir` 可读时为 `"ok"`，不可读/不可用时为 `"degraded"`）、服务标识 `"linke"`、当前发布版本号（当前为 `"V0.65"`）、检查详情 `checks`（包含 `http` 和 `dataDirReadable`）、以及当前 ISO 时间戳 `timestamp`。
