...
> **当前版本：V0.92** — 单机 localhost 原型阶段，新增 Web Console `supervisor-lifecycle-approval-preview-panel` 与手动 `POST /api/supervisor-lifecycle-approval-persistence-preview`，把 V0.90 sanitized approval persistence preview 暴露为浏览器/API 只读预览；只接收粘贴的 config 与可选 approval JSON，不读取本地 approval path，不写 approval 文件，不持久化批准，不返回批准身份、原因、acknowledgement 内容、timestamps、hash 值、路径、URL 或 token，不新增生命周期 apply 面，真实 `--apply` 继续 blocked，Gold 依旧 blocked
