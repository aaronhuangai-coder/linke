...
> **当前版本：V0.86** — 单机 localhost 原型阶段，Web Console `supervisor-install-dry-run-panel` 新增 display-only `rollbackUninstallPlan` 渲染，展示 `Rollback / uninstall plan`、`Rollback / uninstall safety flags`、`buildSupervisorInstallDryRunViewModel rollbackUninstallActions` 与 `buildSupervisorInstallDryRunViewModel rollbackUninstallSafetyLines`；仍固定 `readinessSummary.state:"blocked"`、`installApprovalManifest.approval.approved:false`、`installApprovalManifest.rollback.available:false`、`rollbackUninstallPlan.state:"blocked"` 与 `rollbackUninstallPlan.actions:wouldRun:false`，不调用 launchctl、不读取进程列表、不安装、不启动、不回滚、不卸载、不删除 launchd 文件、不恢复 previous plist、不启动 recovery supervisor、不写 metadata、不连接 NAS、不触发备份/恢复、不执行远程命令，不显示 raw evidence 或 runnable command，Gold 依旧 blocked
