...
- **Gold readiness scorecard** — Web Console 提供 `gold-readiness-panel`，手动 GET `/api/gold-readiness` 展示静态 code-owned capability/blocker scorecard；`agent.js gold-readiness` 可通过同一只读端点输出 sanitized JSON，`--fail-on-blocked` 在 Gold blocked 时退出码 2；V0.84 继续将 `nas-dry-run` (含 CLI fail-on-blocked 自动化门禁)、`security-auth`、`automation-installation` 与 `production-hardening` (含 Web Console `supervisor-install-dry-run-panel` 面板、`POST /api/supervisor-install-dry-run` 接口、`buildSupervisorInstallDryRunViewModel` 视图模型、Agent supervisor-install-dry-run CLI、buildSupervisorInstallDryRunPlan、buildSupervisorInstallReadinessSummary、buildSupervisorInstallCommandPreview、buildSupervisorInstallPreflight、buildSupervisorInstallApprovalManifest、supervisor-install-dry-run --readiness-summary、supervisor-install-dry-run --fail-on-blocked、readinessSummary.state:"blocked"、installCommandPreview.state:"blocked"、installCommandPreview.actions:wouldRun:false、installPreflight.state:"blocked"、installApprovalManifest.state:"blocked"、installApprovalManifest.approval.approved:false、installApprovalManifest.rollback.available:false、Web Console supervisor-status 面板、Agent supervisor-status CLI、GET `/api/supervisor-status`、buildSupervisorStatusResponse、buildSupervisorStatusViewModel、Agent auth-status CLI、Agent Gold readiness CLI、Web Console audit-log 面板、Agent audit-log CLI、Web Console hardening-status 面板、`agent.js hardening-status` 只读 CLI、GET `/api/hardening-status` 硬化状态只读接口与 buildHardeningStatusResponse 响应构建器) 标为 partial，`real-nas-remote-backup` 仍为 Gold blocker
...
- **Supervisor install Web dry-run panel** — Web Console 新增 `supervisor-install-dry-run-panel`，支持手动触发 dry-run-only `POST /api/supervisor-install-dry-run` 并通过 `buildSupervisorInstallDryRunViewModel` 视图模型渲染；无启动请求、不自动轮询，复用 `buildSupervisorInstallDryRunPlan` 展示 sanitized 计划且固定 `readinessSummary.state:"blocked"`、`installApprovalManifest.approval.approved:false` 与 `installApprovalManifest.rollback.available:false`；不调用 launchctl、不读取进程列表、不安装、不启动、不写 metadata、不连接 NAS、不触发备份/恢复、不执行远程命令，不回显 serverUrl、sourcePath、NAS endpoint、credentialRef、token、Authorization、approval identity、timestamp、plist path、executable path 或 runnable command，Gold 依旧 blocked
...
V0.84 增加 Web Console `supervisor-install-dry-run-panel` 与 dry-run-only `POST /api/supervisor-install-dry-run` 接口，用于手动渲染 sanitized supervisor install dry-run 计划，并引入 `buildSupervisorInstallDryRunViewModel` 视图模型。

- **Web Console 面板**：新增 `supervisor-install-dry-run-panel` 面板，仅在操作员点击时手动触发 `POST /api/supervisor-install-dry-run` 请求；无启动请求、不自动轮询。它复用 `buildSupervisorInstallDryRunPlan` 展示 allowlisted blocked 安装计划、command preview、preflight、approval manifest 与 safety flags。
- **视图模型与接口**：新增 dry-run-only 接口 `POST /api/supervisor-install-dry-run`，并使用 `buildSupervisorInstallDryRunViewModel` 处理返回数据。接口固定 `readinessSummary.state:"blocked"`、`installApprovalManifest.approval.approved:false` 与 `installApprovalManifest.rollback.available:false`。
- **脱敏边界**：不回显 config path、sourcePath、serverUrl、NAS endpoint、credentialRef、token、Authorization、approval identity、timestamp、plist path、executable path 或 runnable command 等敏感信息。
- **安全与执行边界**：该面板及接口不调用 launchctl、不读取进程列表、不进行真实安装、不启动服务、不写入 metadata、不写 LaunchAgents/plist/metadata、不建立真实 NAS 连接、不触发备份/恢复、不执行任何远程命令。
