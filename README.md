...
> **当前版本：V0.99** — 单机 localhost 原型阶段，新增只读 `POST /api/supervisor-lifecycle-executor-readiness` API，读取 server-owned approval records 后返回带 `executor-not-implemented-for-action:<actionId>` 的 read-only fail-closed 执行器就绪性评估；该 route 不注册到 `API_WRITE_ROUTES`，read token 可调用，top-level `state:"blocked"` 且 `executorReady:false`，不提交 approval JSON、不写 approval store、不创建 approval storage、不执行 lifecycle apply、不调用 launchctl，`safety` 保持 `readOnly:true`、`lifecycleApplied:false`、`launchctlCalled:false`，Gold 依旧 blocked
...
| V0.99 | 当前版本 | supervisor lifecycle executor readiness API：新增 `POST /api/supervisor-lifecycle-executor-readiness`，组合 server-owned approval records、apply readiness 与 executor readiness，返回 action-driven `executor-not-implemented-for-action:<actionId>` blockers；route 不注册到 `API_WRITE_ROUTES`，read token 可调用，`state:"blocked"` 且 `executorReady:false`；不提交 approval JSON、不写 store、不创建 approval storage、不执行 apply、不调用 launchctl，Gold 依旧 blocked |
