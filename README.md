...
- **Gold readiness scorecard** — Web Console 提供 `gold-readiness-panel`，手动 GET `/api/gold-readiness` 展示静态 code-owned capability/blocker scorecard；`agent.js gold-readiness` 可通过同一只读端点输出 sanitized JSON，`--fail-on-blocked` 在 Gold blocked 时退出码 2；V0.80 继续将 `nas-dry-run` (含 CLI fail-on-blocked 自动化门禁)、`security-auth`、`automation-installation` 与 `production-hardening` (含 Agent supervisor-install-dry-run CLI、buildSupervisorInstallDryRunPlan、buildSupervisorInstallReadinessSummary、supervisor-install-dry-run --readiness-summary、supervisor-install-dry-run --fail-on-blocked、readinessSummary.state:"blocked"、Web Console supervisor-status 面板、Agent supervisor-status CLI、GET `/api/supervisor-status`、buildSupervisorStatusResponse、buildSupervisorStatusViewModel、Agent auth-status CLI、Agent Gold readiness CLI、Web Console audit-log 面板、Agent audit-log CLI、Web Console hardening-status 面板、`agent.js hardening-status` 只读 CLI、GET `/api/hardening-status` 硬化状态只读接口与 buildHardeningStatusResponse 响应构建器) 标为 partial，`real-nas-remote-backup` 仍为 Gold blocker
...
- **Gold 边界**：supervisor-install-dry-run、`buildSupervisorInstallReadinessSummary`、`--readiness-summary` 与 `--fail-on-blocked` 只是 `automation-installation` 与 `production-hardening` 的 partial evidence；真实 installer、launchd install/start、watchdog、监控、recovery supervisor、部署硬化和生产安全边界仍未实现，Gold 发布仍 blocked。
...
- **Gold blockers**：V0.80 基于 V0.79 Agent `supervisor-install-dry-run` CLI、`buildSupervisorInstallDryRunPlan` 与 `test/agent-supervisor-install-dry-run.test.js` sanitized 安装计划 dry-run，继续保留 V0.78 Web Console `supervisor-status-panel`、V0.77 GET `/api/supervisor-status` 与 Agent supervisor-status、V0.76 Agent auth-status、V0.75 Agent Gold readiness、V0.74 Web Console audit-log、V0.73 Agent CLI audit-log、V0.72 Web Console hardening-status、V0.71 Agent `hardening-status`、V0.70 GET /api/hardening-status 和 V0.69 CLI fail-on-blocked / readiness-summary NAS 自动化门禁证据；新增 `buildSupervisorInstallReadinessSummary`、`supervisor-install-dry-run --readiness-summary`、`supervisor-install-dry-run --fail-on-blocked` 与 `readinessSummary.state:"blocked"` 证据。`nas-dry-run`、`security-auth`、`automation-installation` 与 `production-hardening` 继续为 partial，`real-nas-remote-backup` 仍为 blocked。
...
- **NAS、认证和生产硬化仍是 partial**：V0.80 基于 V0.79 Agent `supervisor-install-dry-run` sanitized 安装计划 dry-run、V0.78 Web Console `supervisor-status-panel`、V0.77 GET `/api/supervisor-status` 与 Agent supervisor-status、V0.76 Agent auth-status、V0.75 Agent Gold readiness、V0.74 Web Console audit-log、V0.73 Agent CLI audit-log、V0.72 Web Console hardening-status、V0.71 Agent CLI `hardening-status`、V0.70 GET /api/hardening-status，以及 V0.69 CLI fail-on-blocked 自动化门禁、readiness-summary 摘要输出和 NAS dry-run 执行就绪性摘要；新增 `buildSupervisorInstallReadinessSummary`、`supervisor-install-dry-run --readiness-summary`、`supervisor-install-dry-run --fail-on-blocked` 与 `readinessSummary.state:"blocked"`。这些只提供 sanitized 可见性和 blocker gate，并继续保留 `tokenValuesReturned:false`、`restoreRootValueReturned:false`、`auditPathReturned:false`、`environmentValuesReturned:false`、`supervisor.state:not_configured`、`supervisorInstalled:false` 等脱敏证据；不代表生产硬化完成；没有真实 NAS transport、用户、角色权限、生产级审计、secret management、token 轮换、distributed rate limiting、监控、托管 supervisor 或 recovery supervisor。
...
- ❌ 自动安装或启动守护进程 / 托管 supervisor（当前仅有 `supervisor-status` 只读 `not_configured` 状态基础、`supervisor-install-dry-run` sanitized 安装计划 dry-run 与 blocked readiness gate）
- ❌ 生产环境部署
