...
> **当前版本：V0.95** — 单机 localhost 原型阶段，新增手动 `POST /api/supervisor-lifecycle-approval-persist` 与 Web Console `supervisor-lifecycle-approval-persist-button`，将 V0.94 CLI 与 V0.93 本地 sanitized approval record store 接入 server-owned `dataDir` 的 Web/API 写入路径；该路由注册到 `API_WRITE_ROUTES`，有效批准返回 `201` sanitized `supervisor-lifecycle-approval-record`，invalid approval 返回 `409` sanitized blocked preview，写失败只返回 `failed to persist approval record`；Web action 无启动请求、不自动轮询，不执行 lifecycle apply，不调用 launchctl，Gold 依旧 blocked
...
- **Gold readiness scorecard** — Web Console 提供 `gold-readiness-panel`，手动 GET `/api/gold-readiness` 展示静态 code-owned capability/blocker scorecard；`agent.js gold-readiness` 可通过同一只读端点输出 sanitized JSON，`--fail-on-blocked` 在 Gold blocked 时退出码 2；V0.95 继续将 `nas-dry-run`、`security-auth`、`automation-installation` 与 `production-hardening` 标为 partial，并加入 `POST /api/supervisor-lifecycle-approval-persist`、`test/supervisor-lifecycle-approval-persist-api.test.js`、`supervisor-lifecycle-approval-persist-button`、`test/web-console.test.js`、V0.94 CLI persist、V0.93 local approval store、V0.92 approval preview Web/API、V0.91 approval preview CLI、V0.90 approval persistence preview、V0.89 audit preview、V0.88 fake executor、V0.87 lifecycle apply safety gate、V0.86 rollback/uninstall Web rendering、V0.85 rollbackUninstallPlan、V0.84 supervisor install Web dry-run panel、V0.83 approval manifest、V0.82 preflight gate、V0.81 command preview、V0.80 readiness gate、V0.79 supervisor install CLI 与前序 supervisor/auth/hardening/audit 证据；`real-nas-remote-backup` 仍为 Gold blocker
...
- **NAS、认证和生产硬化仍是 partial**：V0.95 基于 `POST /api/supervisor-lifecycle-approval-persist`、`test/supervisor-lifecycle-approval-persist-api.test.js`、`supervisor-lifecycle-approval-persist-button` 与 `test/web-console.test.js supervisor lifecycle approval persist` 证据，继续保留 V0.94 Agent approval persist CLI、V0.93 local approval store、V0.92 approval preview Web/API、V0.87 blocked lifecycle apply foundation、V0.86-V0.79 supervisor dry-run evidence，以及 V0.69 CLI fail-on-blocked 自动化门禁、readiness-summary 摘要输出与 NAS dry-run 执行就绪性摘要；继续保留 Web Console `supervisor-install-dry-run-panel`、`POST /api/supervisor-install-dry-run` 与 `buildSupervisorInstallDryRunViewModel`。这些只提供 sanitized 可见性和 blocker gate，并继续保留 `tokenValuesReturned:false`、`restoreRootValueReturned:false`、`auditPathReturned:false`、`environmentValuesReturned:false`、`supervisor.state:not_configured`、`supervisorInstalled:false` 等脱敏证据；V0.95 的 Web/API approval persist 只证明 server-owned `dataDir` 的 guarded JSONL 写入入口，不代表完整生产 approval workflow。仍没有真实 NAS transport、用户、角色权限、生产级审计、secret management、token 轮换、distributed rate limiting、监控、托管 supervisor、guarded lifecycle approval apply wiring、rollback、uninstall 或 recovery supervisor。
