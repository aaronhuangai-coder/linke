...
> **当前版本：V1.00** — 单机 localhost 原型阶段，新增 Web Console 手动 `supervisor-lifecycle-executor-readiness-button`，通过 `buildSupervisorLifecycleExecutorReadinessViewModel` 调用只读 `POST /api/supervisor-lifecycle-executor-readiness` 并展示 executor readiness fail-closed 结果；请求只提交 operation 与 config，不读取 approval textarea、不提交 approval JSON，UI 固定展示 `executorReady:false` / `readOnly:true` / `lifecycleApplied:false` / `launchctlCalled:false`，仅显示 action-driven blocker、gate 与 safety flag，不显示 approval identity、reason、hash、路径、URL、token、hostname、username、process id 或 runnable command，Gold 依旧 blocked
