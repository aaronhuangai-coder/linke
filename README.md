...
- **Gold readiness scorecard** — Web Console 提供 `gold-readiness-panel`，手动 GET `/api/gold-readiness` 展示静态 code-owned capability/blocker scorecard；`agent.js gold-readiness` 可通过同一只读端点输出 sanitized JSON，`--fail-on-blocked` 在 Gold blocked 时退出码 2；V0.79 继续将 `nas-dry-run` (含 CLI fail-on-blocked 自动化门禁)、`security-auth`、`automation-installation` 与 `production-hardening` (含 Agent supervisor-install-dry-run CLI、buildSupervisorInstallDryRunPlan、Web Console supervisor-status 面板、Agent supervisor-status CLI、GET `/api/supervisor-status`、buildSupervisorStatusResponse、buildSupervisorStatusViewModel、Agent auth-status CLI、Agent Gold readiness CLI、Web Console audit-log 面板、Agent audit-log CLI、Web Console hardening-status 面板、`agent.js hardening-status` 只读 CLI、GET `/api/hardening-status` 硬化状态只读接口与 buildHardeningStatusResponse 响应构建器) 标为 partial，`real-nas-remote-backup` 仍为 Gold blocker
...
V0.79 增加本地 `agent.js supervisor-install-dry-run --config <file>`，用于在不安装、不启动、不写 plist 的前提下输出 sanitized supervisor 安装计划。该命令读取同一份 Linke config，经过 `validateConfig` 后交给 `buildSupervisorInstallDryRunPlan` 生成 JSON；它不访问服务器、不调用 NAS、不执行备份或恢复，也不写入任何安装文件。

- **Agent CLI**：`agent.js supervisor-install-dry-run` 只接受 `--config` 输入；不支持 --output，不会创建 plist 或任何输出文件；读取或校验配置失败时只返回固定脱敏错误，不回显 config path 或配置原值。需要单独预览 launchd plist 时，继续使用 `launchd-dry-run`。
...
- **Gold 边界**：supervisor-install-dry-run 只是 `automation-installation` 与 `production-hardening` 的 partial evidence；真实 installer、launchd install/start、watchdog、监控、recovery supervisor、部署硬化和生产安全边界仍未实现，Gold 发布仍 blocked。
...
- **Gold blockers**：V0.79 基于 V0.78 (包含 Web Console `supervisor-status-panel` 与 `buildSupervisorStatusViewModel` 手动只读状态展示)、V0.77 (包含 GET `/api/supervisor-status` 与 Agent supervisor-status 只读状态基础)、V0.76 (包含 Agent auth-status 只读认证状态 CLI)、V0.75 (包含 Agent Gold readiness 只读自动化门禁)、V0.74 (包含 Web Console audit-log 手动只读面板)、V0.73 (包含 Agent CLI audit-log 只读包装)、V0.72 (包含 Web Console hardening-status 手动只读面板)、V0.71 (包含 `agent.js hardening-status` 只读 CLI)、V0.70 (包含 GET /api/hardening-status 硬化状态只读接口与 buildHardeningStatusResponse 响应构建器) 和 V0.69 (包含 NAS CLI fail-on-blocked 自动化门禁、readiness-summary 摘要输出、网页执行门禁、凭证引用展示基础、15 个 credential-like 字段精确拒绝、`readinessSummary` 与 per-target `executionReadiness` dry-run 执行就绪性摘要) 基础，新增 Agent `supervisor-install-dry-run` CLI、`buildSupervisorInstallDryRunPlan` 与 `test/agent-supervisor-install-dry-run.test.js` sanitized 安装计划 dry-run 证据，继续将 `nas-dry-run`、`security-auth`、`automation-installation` 与 `production-hardening` 标为 partial，明确当前只有 NAS `credentialRef` 非密钥引用基础、`validateNasCredentialRef`、`ALLOWED_NAS_CREDENTIAL_REF_PATTERN`、`FORBIDDEN_NAS_CREDENTIAL_FIELDS` 15 个 exact-key denylist、`credentialRefConfigured` sanitized 输出与网页展示、`executionGate` 网页展示与固定 `remoteExecutionAllowed:false`、固定 blocker codes 只读展示、可选 API Bearer token 骨架、`LINKE_READ_TOKEN` / `LINKE_WRITE_TOKEN` 读写 token foundation、`API_WRITE_ROUTES` 共享写入路由注册表、`isApiWriteRoute` 同一来源授权判断、`formatApiRoute` 与 GET `/api/auth-status` 的 `writeRoutes` 状态报告对齐、403 `Forbidden` / `auth.forbidden` 越权拒绝、GET `/api/auth-status` auth-status readiness API、GET `/api/supervisor-status` supervisor 状态基础、GET /api/hardening-status 只读硬化状态接口、`buildAuthStatusResponse`、`buildSupervisorStatusResponse`、`buildSupervisorStatusViewModel`、`buildSupervisorInstallDryRunPlan`、`buildHardeningStatusResponse`、configuredScopes / `writeRoutes` sanitized 响应、`tokenValuesReturned:false`、`supervisor.state:not_configured`、`supervisorInstalled:false`、`restoreRootValueReturned:false`、`auditPathReturned:false`、`environmentValuesReturned:false`、`successAuditEvent:false`、Agent CLI token 支持、Agent CLI auth-status、Agent CLI supervisor-status、Agent CLI supervisor-install-dry-run、Agent CLI gold-readiness gate、Agent CLI audit-log sanitized 只读查询、Web Console supervisor-status 面板、Web Console audit-log 面板、Web Console 内存态 token UX、Web Console hardening-status 面板、1 MiB 请求体上限、未知 500 `Internal Server Error` 脱敏、可选 `LINKE_RESTORE_ROOT` 恢复目标 guard、恢复目标 symlink 写入防护、本地 audit log foundation、可选 API rate-limit foundation 和可选 audit retention foundation；`real-nas-remote-backup` 仍为 blocked。
...
- **NAS、认证和生产硬化仍是 partial**：V0.79 基于 V0.78 Web Console `supervisor-status-panel` 与 `buildSupervisorStatusViewModel` 手动只读状态展示、V0.77 GET `/api/supervisor-status` 与 Agent supervisor-status 只读状态基础、V0.76 Agent auth-status 只读认证状态 CLI、V0.75 Agent Gold readiness CLI 只读自动化门禁、V0.74 Web Console audit-log 手动只读面板、V0.73 Agent CLI audit-log 只读包装、V0.72 Web Console hardening-status 手动只读面板、V0.71 Agent CLI `hardening-status` 只读包装、V0.70 GET /api/hardening-status 硬化状态只读接口与 buildHardeningStatusResponse 响应构建器，并新增 Agent `supervisor-install-dry-run` CLI、`buildSupervisorInstallDryRunPlan` 与 `test/agent-supervisor-install-dry-run.test.js` sanitized 安装计划 dry-run；这些只提供 sanitized 可见性和 blocker gate，不代表生产硬化完成。当前仍只有 V0.69 CLI fail-on-blocked 自动化门禁、readiness-summary 摘要输出、NAS `credentialRef` 非密钥引用基础与网页展示、`credentialRefConfigured` sanitized dry-run 输出与网页展示、15 个 credential-like 字段精确拒绝、固定 `executionGate` 远程执行阻塞与网页展示、`readinessSummary` 与 per-target `executionReadiness` dry-run 执行就绪性摘要、固定 blocker codes 只读展示、`/api/*` 的可选 Bearer token 检查、`LINKE_READ_TOKEN` / `LINKE_WRITE_TOKEN` 读写 token foundation、`API_WRITE_ROUTES` 共享写入路由注册表、`isApiWriteRoute` 同一来源授权判断、`formatApiRoute` 与 GET `/api/auth-status` 的 `writeRoutes` 状态报告对齐、403 `Forbidden` / `auth.forbidden` 越权拒绝、GET `/api/auth-status` 认证状态只读接口、GET `/api/supervisor-status` supervisor 状态基础、configuredScopes / `writeRoutes` sanitized 响应、`tokenValuesReturned:false`、`supervisor.state:not_configured`、`supervisorInstalled:false`、`launchdFileWritten:false`、`restoreRootValueReturned:false`、`auditPathReturned:false`、`environmentValuesReturned:false`、`successAuditEvent:false`、Agent CLI `--token`、Agent CLI auth-status、Agent CLI supervisor-status、Agent CLI supervisor-install-dry-run、Agent CLI gold-readiness、Agent CLI audit-log、Web Console supervisor-status 面板、Web Console audit-log 面板、Web Console 内存态 token UX、1 MiB 请求体上限、未知 500 `Internal Server Error` 脱敏、可选 `LINKE_RESTORE_ROOT` 恢复目标 guard、恢复目标 symlink 写入防护、本地 audit log foundation、可选 API rate-limit foundation 和可选 audit retention foundation；没有真实 NAS transport、用户、角色权限、生产级审计、secret management、token 轮换、distributed rate limiting、监控、托管 supervisor 或 recovery supervisor。
...
- ❌ 自动安装或启动守护进程 / 托管 supervisor（当前仅有 `supervisor-status` 只读 `not_configured` 状态基础与 `supervisor-install-dry-run` sanitized 安装计划 dry-run）
- ❌ 生产环境部署
