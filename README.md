...
> **当前版本：V0.94** — 单机 localhost 原型阶段，新增 `agent.js supervisor-lifecycle-approval-persist` 显式 CLI，将 V0.93 本地 sanitized approval record store 接入既有 approval persistence preview；仅在 operator 传入 `--config`、`--operation`、`--approval` 与显式 `--data-dir` 后写入 sanitized JSONL record；invalid approval 只输出 sanitized blocked preview 并退出 2，不创建 approval store；不保存 approvedBy 值、approval reason、acknowledgement 内容、approval timestamps、hash 值、路径、URL 或 token；不新增 Web/API persist 面，不执行 lifecycle apply，不调用 launchctl，Gold 依旧 blocked
...
- **Agent CLI**：V0.75 增加 `agent.js gold-readiness` 只读命令，通过 GET `/api/gold-readiness` 输出 sanitized JSON scorecard；支持 `--token <token>`；支持 `--fail-on-blocked`，当返回 `status:"blocked"` 时退出码为 2，仍正常打印 JSON。V0.76 另增加 `agent.js auth-status`，用于只读查看 sanitized auth scope 状态；V0.77 增加 `agent.js supervisor-status`，用于只读查看 sanitized `not_configured` supervisor 状态；V0.78 增加 Web Console `supervisor-status-panel` 手动只读状态面板；V0.79 增加 `agent.js supervisor-install-dry-run` 本地 sanitized 安装计划 dry-run；V0.80 增加 `agent.js supervisor-install-dry-run --readiness-summary / --fail-on-blocked` blocked readiness gate；V0.81 增加 full JSON `installCommandPreview` 非 runnable command preview；V0.82 增加 full JSON `installPreflight` 安装前预检门禁；V0.83 增加 full JSON `installApprovalManifest` 批准与 rollback manifest；V0.85 增加 full JSON `rollbackUninstallPlan`；V0.86 增加 display-only Web Console `rollbackUninstallPlan` rendering；V0.87 增加 `supervisor-lifecycle-apply` 运行逻辑基础；V0.88 增加 `executeSupervisorLifecycleApply` 单元测试执行契约；V0.89 增加 `buildSupervisorLifecycleAuditPreview` 审计预览；V0.90 增加 `buildSupervisorLifecycleApprovalPersistencePreview` 批准持久化预览；V0.91 增加 `agent.js supervisor-lifecycle-approval-persistence-preview` 只读 CLI；V0.92 增加 Web Console `supervisor-lifecycle-approval-preview-panel` 与 `POST /api/supervisor-lifecycle-approval-persistence-preview` 手动只读预览；V0.93 增加 `src/approval-store.js` 本地 sanitized approval record store foundation；V0.94 增加 `agent.js supervisor-lifecycle-approval-persist` 显式本地 approval record persist CLI。
...
- **Gold blockers**：V0.94 基于 `src/agent.js supervisor-lifecycle-approval-persist` 与 `test/agent-supervisor-lifecycle-approval-persist.test.js` 证据，继续保留 V0.93 `src/approval-store.js`、`buildSupervisorLifecycleApprovalRecord`、`appendSupervisorLifecycleApprovalRecord`、`readSupervisorLifecycleApprovalRecords` 与 `test/approval-store.test.js` 证据，继续保留 V0.92 Web Console `supervisor-lifecycle-approval-preview-panel`、`POST /api/supervisor-lifecycle-approval-persistence-preview`、`buildSupervisorLifecycleApprovalPersistencePreviewViewModel` 与 `test/web-console.test.js` 证据，继续保留 V0.91 `agent.js supervisor-lifecycle-approval-persistence-preview` 只读 CLI 与 `test/agent-supervisor-lifecycle-approval-persistence-preview.test.js` 证据，继续保留 V0.90 `buildSupervisorLifecycleApprovalPersistencePreview` 批准持久化预览与 `test/supervisor-lifecycle-approval-persistence-preview.test.js` 证据，继续保留 V0.89 `buildSupervisorLifecycleAuditPreview` 审计预览与 `test/supervisor-lifecycle-audit-preview.test.js` 证据，继续保留 V0.88 `executeSupervisorLifecycleApply` 单元测试执行契约与 `test/supervisor-lifecycle-executor.test.js` 证据，继续保留 V0.87 `src/supervisor-lifecycle.js`、`test/supervisor-lifecycle.test.js`、`src/agent.js supervisor-lifecycle-apply`、`test/agent-supervisor-lifecycle-apply.test.js` 与 `supervisorLifecycleApply.state:blocked` 证据，继续保留 V0.86 display-only `rollbackUninstallPlan Web Console rendering`、`buildSupervisorInstallDryRunViewModel rollbackUninstallActions`、`buildSupervisorInstallDryRunViewModel rollbackUninstallSafetyLines`、V0.85 `buildSupervisorRollbackUninstallPlan` 与 `rollbackUninstallPlan.state:blocked` 以及 `rollbackUninstallPlan.actions:wouldRun:false` 证据，继续保留 V0.84 基于 V0.82 installPreflight、V0.81 non-runnable command preview、V0.80 blocked readiness gate、V0.79 Agent `supervisor-install-dry-run` CLI、`buildSupervisorInstallDryRunPlan` 与 `test/agent-supervisor-install-dry-run.test.js` sanitized 安装计划 dry-run，继续保留 V0.78 Web Console `supervisor-status-panel`、V0.77 GET `/api/supervisor-status` 与 Agent supervisor-status、V0.76 Agent auth-status、V0.75 Agent Gold readiness、V0.74 Web Console audit-log、V0.73 Agent CLI audit-log、V0.72 Web Console hardening-status、V0.71 Agent `hardening-status`、V0.70 GET /api/hardening-status 和 V0.69 CLI fail-on-blocked / readiness-summary NAS 自动化门禁证据；继续保留 Web Console `supervisor-install-dry-run-panel`、`POST /api/supervisor-install-dry-run` 与 `buildSupervisorInstallDryRunViewModel` 证据。`nas-dry-run`、`security-auth`、`automation-installation` 与 `production-hardening` 继续为 partial，`real-nas-remote-backup` 仍为 blocked。
...
- **NAS、认证和生产硬化仍是 partial**：V0.94 基于 `src/agent.js supervisor-lifecycle-approval-persist` 与 `test/agent-supervisor-lifecycle-approval-persist.test.js` 证据，继续保留 V0.93 `src/approval-store.js`、`buildSupervisorLifecycleApprovalRecord`、`appendSupervisorLifecycleApprovalRecord`、`readSupervisorLifecycleApprovalRecords` 与 `test/approval-store.test.js` 证据，继续保留 V0.92 Web Console `supervisor-lifecycle-approval-preview-panel`、`POST /api/supervisor-lifecycle-approval-persistence-preview`、`buildSupervisorLifecycleApprovalPersistencePreviewViewModel` 与 `test/web-console.test.js` 证据，继续保留 V0.91 `agent.js supervisor-lifecycle-approval-persistence-preview` 只读 CLI 与 `test/agent-supervisor-lifecycle-approval-persistence-preview.test.js` 证据，继续保留 V0.90 `buildSupervisorLifecycleApprovalPersistencePreview` 批准持久化预览与 `test/supervisor-lifecycle-approval-persistence-preview.test.js` 证据，继续保留 V0.89 `buildSupervisorLifecycleAuditPreview` 审计预览与 `test/supervisor-lifecycle-audit-preview.test.js` 证据，继续保留 V0.88 `executeSupervisorLifecycleApply` 与 `test/supervisor-lifecycle-executor.test.js` 单元测试执行契约证据，继续保留 V0.87 `src/supervisor-lifecycle.js`、`test/supervisor-lifecycle.test.js`、`src/agent.js supervisor-lifecycle-apply`、`test/agent-supervisor-lifecycle-apply.test.js` 与 `supervisorLifecycleApply.state:blocked` 证据，继续保留 V0.86 display-only `rollbackUninstallPlan Web Console rendering`、`buildSupervisorInstallDryRunViewModel rollbackUninstallActions`、`buildSupervisorInstallDryRunViewModel rollbackUninstallSafetyLines`，继续保留 V0.85 `rollbackUninstallPlan.actions:wouldRun:false`、V0.84 基于 V0.83 installApprovalManifest、V0.82 installPreflight、V0.81 non-runnable command preview、V0.80 blocked readiness gate、V0.79 Agent `supervisor-install-dry-run` sanitized 安装计划 dry-run、V0.78 Web Console `supervisor-status-panel`、V0.77 GET `/api/supervisor-status` 与 Agent supervisor-status、V0.76 Agent auth-status、V0.75 Agent Gold readiness、V0.74 Web Console audit-log、V0.73 Agent CLI audit-log、V0.72 Web Console hardening-status、V0.71 Agent CLI `hardening-status`、V0.70 GET /api/hardening-status，以及 V0.69 CLI fail-on-blocked 自动化门禁、readiness-summary 摘要输出与 NAS dry-run 执行就绪性摘要；继续保留 Web Console `supervisor-install-dry-run-panel`、`POST /api/supervisor-install-dry-run` 与 `buildSupervisorInstallDryRunViewModel`。这些只提供 sanitized 可见性和 blocker gate，并继续保留 `tokenValuesReturned:false`、`restoreRootValueReturned:false`、`auditPathReturned:false`、`environmentValuesReturned:false`、`supervisor.state:not_configured`、`supervisorInstalled:false` 等脱敏证据；V0.94 的 Agent approval persist CLI 只证明显式 `--data-dir` 本地 JSONL 写入入口，不代表完整生产 approval workflow。仍没有真实 NAS transport、用户、角色权限、生产级审计、secret management、token 轮换、distributed rate limiting、监控、托管 supervisor、guarded lifecycle approval apply wiring、rollback、uninstall 或 recovery supervisor。
