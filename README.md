...
- **Gold 边界**：supervisor-install-dry-run、`buildSupervisorInstallReadinessSummary`、`buildSupervisorInstallCommandPreview`、`buildSupervisorInstallPreflight`、`buildSupervisorInstallApprovalManifest`、`--readiness-summary`、`--fail-on-blocked`、`readinessSummary.state:"blocked"`、`installCommandPreview.state:"blocked"`、`installPreflight.state:"blocked"`、`installApprovalManifest.state:"blocked"`、`installApprovalManifest.approval.approved:false` 与 `installApprovalManifest.rollback.available:false` 只是 `automation-installation` 与 `production-hardening` 的 partial evidence；真实 installer、launchd install/start、watchdog、监控、批准持久化、rollback、uninstall、recovery supervisor、部署硬化和生产安全边界仍未实现，Gold 发布仍 blocked。
...
- **NAS、认证和生产硬化仍是 partial**：V0.84 基于 V0.83 installApprovalManifest、V0.82 installPreflight、V0.81 non-runnable command preview、V0.80 blocked readiness gate、V0.79 Agent `supervisor-install-dry-run` sanitized 安装计划 dry-run、V0.78 Web Console `supervisor-status-panel`、V0.77 GET `/api/supervisor-status` 与 Agent supervisor-status、V0.76 Agent auth-status、V0.75 Agent Gold readiness、V0.74 Web Console audit-log、V0.73 Agent CLI audit-log、V0.72 Web Console hardening-status、V0.71 Agent CLI `hardening-status`、V0.70 GET /api/hardening-status，以及 V0.69 CLI fail-on-blocked 自动化门禁、readiness-summary 摘要输出和 NAS dry-run 执行就绪性摘要；新增 Web Console `supervisor-install-dry-run-panel`、`POST /api/supervisor-install-dry-run` 与 `buildSupervisorInstallDryRunViewModel`。这些只提供 sanitized 可见性和 blocker gate，并继续保留 `tokenValuesReturned:false`、`restoreRootValueReturned:false`、`auditPathReturned:false`、`environmentValuesReturned:false`、`supervisor.state:not_configured`、`supervisorInstalled:false` 等脱敏证据；不代表生产硬化完成；没有真实 NAS transport、用户、角色权限、生产级审计、secret management、token 轮换、distributed rate limiting、监控、托管 supervisor、批准持久化、rollback、uninstall 或 recovery supervisor。
