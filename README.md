...
| V0.91 | 当前版本 | supervisor lifecycle approval persistence preview CLI：新增 `agent.js supervisor-lifecycle-approval-persistence-preview` 只读 CLI，将 V0.90 `buildSupervisorLifecycleApprovalPersistencePreview(plan, approval, options)` sanitized 批准持久化预览暴露给 operator；只读取显式 `--config` 与可选 `--approval`，拒绝 `--apply`，支持 `--fail-on-blocked`，不写 approval 文件，不返回批准身份、原因、acknowledgement 内容、timestamps、hash 值、路径、URL 或 token，不新增 Web/API 生命周期 apply 面，真实 `--apply` 继续 blocked，Gold 依旧 blocked |
