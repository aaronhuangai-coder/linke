...
> **当前版本：V0.83** — 单机 localhost 原型阶段，新增 `agent.js supervisor-install-dry-run` full JSON 内的 sanitized `installApprovalManifest` 批准与 rollback manifest，固定 `installApprovalManifest.state:"blocked"`、`installApprovalManifest.approval.approved:false` 与 `installApprovalManifest.rollback.available:false`
...
- **Gold blockers**：V0.83 基于 V0.82 installPreflight、V0.81 non-runnable command preview、V0.80 blocked readiness gate、V0.79 Agent `supervisor-install-dry-run` CLI、`buildSupervisorInstallDryRunPlan` 与 `test/agent-supervisor-install-dry-run.test.js` sanitized 安装计划 dry-run，继续保留 V0.78 Web Console `supervisor-status-panel`、V0.77 GET `/api/supervisor-status` 与 Agent supervisor-status、V0.76 Agent auth-status、V0.75 Agent Gold readiness、V0.74 Web Console audit-log、V0.73 Agent CLI audit-log、V0.72 Web Console hardening-status、V0.71 Agent `hardening-status`、V0.70 GET /api/hardening-status 和 V0.69 CLI fail-on-blocked / readiness-summary NAS 自动化门禁证据；新增 `buildSupervisorInstallApprovalManifest`、`installApprovalManifest.state:"blocked"`、`installApprovalManifest.approval.approved:false` 与 `installApprovalManifest.rollback.available:false` 证据。`nas-dry-run`、`security-auth`、`automation-installation` 与 `production-hardening` 继续为 partial，`real-nas-remote-backup` 仍为 blocked。
...
- **NAS、认证和生产硬化仍是 partial**：V0.83 基于 V0.82 installPreflight、V0.81 non-runnable command preview、V0.80 blocked readiness gate、V0.79 Agent `supervisor-install-dry-run` sanitized 安装计划 dry-run、V0.78 Web Console `supervisor-status-panel`、V0.77 GET `/api/supervisor-status` 与 Agent supervisor-status、V0.76 Agent auth-status、V0.75 Agent Gold readiness、V0.74 Web Console audit-log、V0.73 Agent CLI audit-log、V0.72 Web Console hardening-status、V0.71 Agent CLI `hardening-status`、V0.70 GET /api/hardening-status，以及 V0.69 CLI fail-on-blocked 自动化门禁、readiness-summary 摘要输出和 NAS dry-run 执行就绪性摘要；新增 `buildSupervisorInstallApprovalManifest`、`installApprovalManifest.state:"blocked"`、`installApprovalManifest.approval.approved:false` 与 `installApprovalManifest.rollback.available:false`。这些只提供 sanitized 可见性和 blocker gate，并继续保留 `tokenValuesReturned:false`、`restoreRootValueReturned:false`、`auditPathReturned:false`、`environmentValuesReturned:false`、`supervisor.state:not_configured`、`supervisorInstalled:false` 等脱敏证据；不代表生产硬化完成；没有真实 NAS transport、用户、角色权限、生产级审计、secret management、token 轮换、distributed rate limiting、监控、托管 supervisor、批准持久化、rollback、uninstall 或 recovery supervisor。
