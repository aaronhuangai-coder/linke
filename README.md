...
> **当前版本：V0.87** — 单机 localhost 原型阶段，新增 `supervisor-lifecycle-apply` 运行逻辑基础；未带 `--apply` 时默认为 dry-run/blocked，即使带了 `--apply` 仍由于所有门禁未通过且缺少执行器而保持 blocked 状态；`recover` 恢复模式仍固定 blocked；不提供 Web 端生命周期执行按钮或 API 应用端点；不进行任何生产级部署声明、Gold 级别声明或恢复就绪声明；不代表真实主机修改；新增 `src/supervisor-lifecycle.js`、`test/supervisor-lifecycle.test.js`、`src/agent.js supervisor-lifecycle-apply`、`test/agent-supervisor-lifecycle-apply.test.js` 与 `supervisorLifecycleApply.state:blocked` 证据；Gold 依旧 blocked
...
| V0.87 | 当前版本 | supervisor lifecycle apply safety gate：Agent CLI 新增 `supervisor-lifecycle-apply` 指令基础，在未带 `--apply` 时默认为 dry-run/blocked 计划，即使带了 `--apply` 仍因所有门禁未通过且缺少执行器 (executor-implementation-missing) 而固定保持 blocked 状态；`recover` 恢复模式仍固定 blocked；不提供 Web 端生命周期执行按钮或 API 应用端点；不进行任何生产部署完成、Gold 发布完成或恢复就绪声明；不代表真实主机修改，Gold 依旧 blocked |
