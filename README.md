...
> **当前版本：V0.96** — 单机 localhost 原型阶段，新增手动 `POST /api/supervisor-lifecycle-apply-readiness` 与 Web Console `supervisor-lifecycle-apply-readiness-button`，读取 server-owned approval records 后生成 read-only fail-closed apply readiness preflight；该路由不注册到 `API_WRITE_ROUTES`，read token 可调用，返回 `supervisor-lifecycle-apply-readiness`、approval record gate 状态、record count、gate 布尔、next blocker 与 safety flags；不提交 approval JSON、不执行 lifecycle apply、不调用 launchctl、不写 metadata、不连接 NAS，Gold 依旧 blocked
