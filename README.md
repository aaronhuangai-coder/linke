...
| V0.64 | 当前版本 | NAS credential reference gate：新增 `credentialRef` 非密钥 slug 校验、`credentialRefConfigured` sanitized dry-run 输出与固定 `executionGate`，真实 NAS 传输仍 blocked |
...
- **NAS dry-run** — API、CLI 与 Web Console 可验证 Synology / Ugreen 目标配置并输出计划；V0.64 支持 `credentialRef` 非密钥引用基础并仅在输出中返回 `credentialRefConfigured`，不连接 NAS、不写远端、不保存凭证、不回显引用原值
...
- **Gold readiness scorecard** — Web Console 提供 `gold-readiness-panel`，手动 GET `/api/gold-readiness` 展示静态 code-owned capability/blocker scorecard；V0.64 将 `nas-dry-run`、`security-auth` 与 `production-hardening` 标为 partial，`real-nas-remote-backup` 仍为 Gold blocker
...
- 返回的健康数据包含：`status`（当 `dataDir` 可读时为 `"ok"`，不可读/不可用时为 `"degraded"`）、服务标识 `"linke"`、当前发布版本号（当前为 `"V0.64"`）、检查详情 `checks`（包含 `http` 和 `dataDirReadable`）、以及当前 ISO 时间戳 `timestamp`。
