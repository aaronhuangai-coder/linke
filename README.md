...
> **当前版本：V0.93** — 单机 localhost 原型阶段，新增 `src/approval-store.js` 本地 sanitized approval record store foundation，可在显式 `dataDir` 中追加/读取 approval JSONL 记录；只持久化 record id、createdAt、operation、approvalValid、acknowledgement count、validation 布尔和 safety flags，不保存 approvedBy 值、approval reason、acknowledgement 内容、approval timestamps、hash 值、路径、URL 或 token；不新增 Web/API persist 面，不接入 lifecycle apply，不调用 launchctl，Gold 依旧 blocked
...
- **Gold readiness scorecard** — Web Console 提供 `gold-readiness-panel`，手动 GET `/api/gold-readiness` 展示静态 code-owned capability/blocker scorecard；`agent.js gold-readiness` 可通过同一只读端点输出 sanitized JSON，`--fail-on-blocked` 在 Gold blocked 时退出码 2；V0.86 继续将 `nas-dry-run` (含 CLI fail-on-blocked 自动化门禁)、`security-auth`、`automation-installation` 与 `production-hardening` (含 `rollbackUninstallPlan Web Console rendering`、`buildSupervisorInstallDryRunViewModel rollbackUninstallActions`、`buildSupervisorInstallDryRunViewModel rollbackUninstallSafetyLines`、Web Console `supervisor-install-dry-run-panel` 面板、`POST /api/supervisor-install-dry-run` 接口、`buildSupervisorInstallDryRunViewModel` 视图模型、Agent supervisor-install-dry-run CLI、buildSupervisorInstallDryRunPlan、buildSupervisorInstallReadinessSummary、buildSupervisorInstallCommandPreview、buildSupervisorInstallPreflight、buildSupervisorInstallApprovalManifest、buildSupervisorRollbackUninstallPlan、supervisor-install-dry-run --readiness-summary、supervisor-install-dry-run --fail-on-blocked、readinessSummary.state:"blocked"、installCommandPreview.state:"blocked"、installCommandPreview.actions:wouldRun:false、installPreflight.state:"blocked"、installApprovalManifest.state:"blocked"、installApprovalManifest.approval.approved:false、installApprovalManifest.rollback.available:false、rollbackUninstallPlan.state:"blocked"、rollbackUninstallPlan.actions:wouldRun:false、Web Console supervisor-status 面板、Agent supervisor-status CLI、GET `/api/supervisor-status`、buildSupervisorStatusResponse、buildSupervisorStatusViewModel、Agent auth-status CLI、Agent Gold readiness CLI、Web Console audit-log 面板、Agent audit-log CLI、Web Console hardening-status 面板、`agent.js hardening-status` 只读 CLI、GET `/api/hardening-status` 硬化状态只读接口与 buildHardeningStatusResponse 响应构建器) 标为 partial，`real-nas-remote-backup` 仍为 Gold blocker
