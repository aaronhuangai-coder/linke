...
- Existing file fail-closed JSONL algorithm design §8.2 exact (1.5 MiB / `1_572_864` UTF-8 file-size bytes via `safeReadText`/`stat.size`, not JS `string.length`; **existing** 4096-line validation bound — **not** retention/cap; existing 4096 may append → 4097 lines; next preflight with existing 4097 rejects; no delete/compaction/rotation). Contract correction: frozen 1 MiB could not hold 4096 full canonical lines; operation/actionId are exact `SUPERVISOR_LIFECYCLE_OPERATION_ACTION_IDS` SoT enums so per-line UTF-8 upper bound is verifiable (current max 328 B; `328 * 4097 < 1_572_864`).
...
export const CAPABILITY_REAL_AUDIT_MAX_PRE_READ_BYTES = 1_572_864; // 1.5 MiB = 1536 * 1024
...
**JSONL preflight algorithm（与 design §8.2 同步冻结；1.5 MiB / `1_572_864` file-size-byte bound）：**

Bound 说明：`CAPABILITY_REAL_AUDIT_MAX_PRE_READ_BYTES` 是 `safeReadText` / file `stat.size` 的 UTF-8 **file-size bytes**，不是 JS `string.length`。Contract correction：原先 1 MiB 无法容纳 4096 完整 canonical 行（当前 SoT 最大行 328 B；`328 * 4097 = 1_343_816 < 1_572_864`）；`operation`/`actionId` 由 `SUPERVISOR_LIFECYCLE_OPERATION_ACTION_IDS` 精确枚举，故行上界可验证。
...
13. oversize content > 1.5 MiB（`CAPABILITY_REAL_AUDIT_MAX_PRE_READ_BYTES + 1` byte）→ reject
...
4. Inside **one** queue task: try `safeReadText` with `maxBytes: CAPABILITY_REAL_AUDIT_MAX_PRE_READ_BYTES` (`1_572_864` / 1.5 MiB file-size bytes); ENOENT → treat as missing; other SafeDataFileError → sink-invalid.
